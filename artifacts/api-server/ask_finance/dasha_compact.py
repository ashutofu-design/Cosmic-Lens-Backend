"""Compact finance dasha for Engine Execution + LLM (timing asks only).

Sensitive lords = 2/5/9/11 + 6/8/12 occupants & house lords + Jupiter/Venus.
"""
...
_SENSITIVE_HOUSES = frozenset({2, 5, 6, 8, 9, 11, 12})
...
def compute_finance_dasha_compact(
    kundli: dict,
    *,
    ...
    if not isinstance(kundli, dict) or not kundli:
        return {
            "schema_version": "finance_dasha_compact_v1",
            "error": "empty_kundli",
            "current": None,
            "top_windows": [],
        }
    ...
    for score, row in scored:
        ...
        top.append({
            **row,
            ...
            "score": round(score, 1),
            "why": (
                "2/5/9/11 or leak houses / Jupiter-Venus active in MD/AD/PD"
                if score > 0
