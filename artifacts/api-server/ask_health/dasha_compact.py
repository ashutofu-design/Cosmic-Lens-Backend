"""Compact health dasha for Engine Execution + LLM (timing asks only).
...
_SENSITIVE_HOUSES = frozenset({6, 8, 12})
...
def _sensitive_planets(kundli: dict) -> set[str]:
    """6/8/12 occupants + dusthana house lords (D1) — light timing relevance."""
    out: set[str] = set()
...
def compute_health_dasha_compact(
    kundli: dict,
    *,
    ...
    if not isinstance(kundli, dict) or not kundli:
        return {
            "schema_version": "health_dasha_compact_v1",
            "error": "empty_kundli",
            "current": None,
            "top_windows": [],
        }
    ...
    for w in chain:
        ...
        # Prefer upcoming (or current) sensitive windows; keep chronological ties
    ...
    for score, row in scored:
        ...
        top.append({
            **row,
            ...
            "score": round(score, 1),
            "why": (
                "6/8/12 lord or occupant active in MD/AD/PD"
                if score > 0
...
def maybe_attach_dasha_compact(
    ...
    *,
    ...
    """Attach dasha_timing_compact only for health timing questions."""
    ...
    try:
        ...
        if not is_health_timing_question(q, llm_intent):
            pack.pop("dasha_timing_compact", None)
            return pack
    except Exception:
        return pack

    pack["dasha_timing_compact"] = compute_health_dasha_compact(kundli)
    return pack
