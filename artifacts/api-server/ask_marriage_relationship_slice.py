...
_MR_DOMAIN_RX = re.compile(
    ...
    r"serious|casual|long[\s-]*term|time\s*pass|genuine|exclusive|priorit\w*|"
    r"sacrifice|compromise|phobic|nibha\w*|maintain|responsibility|official|"
    r"confused|unavailable|convenience|lifelong|invest|effort|promises?|pakka|"
    ...
    r"companion|companionship|loneliness|lonely|akele\s*pan|akela\s*pan|akela|"
    ...
    r"support|saath\s*dega|saath\s*degi|saath\s*denge|saath\s*denge|communication|conflict|teamwork|compatible|compatibility|compat|match|values?|life\s*goals?|goals?|expectations?|understanding|bonding|respect|space|healthy|balanced|successful|lifetime|gun\s*milan|"
    ...
    r"obsess|jealous|jealousy|possessive|secret|hidden|chhup|affair|"
...
def _build_partner_nature_minimal_block(
    ...
    lines.extend([
        "",
        "=== OUTPUT FORMAT (mandatory — internal guide; NEVER show labels to user) ===",
        "Write exactly 3 short Hinglish prose paragraphs (~90–120 words total).",
        "First paragraph: blend 7H sign + occupants → emotional/social nature.",
        "Second paragraph: 7th lord placement → mindset, communication, intensity.",
        f"Third paragraph: {karak_name} karak → look, presence, lifestyle.",
        "User must see ONLY plain flowing text — NO Step/Para labels, NO headings,",
        "NO bullets, NO boxes, NO blockquotes, NO markdown section titles.",
        "Example start: 'Aapke chart ke mutabik, aapke partner kafi intelligent...'",
        "Tone: insightful, positive, warm. USE ho sakta hai / lagta hai / shayad ONLY.",
        "NEVER pakka hoga / yahi hoga / milega hi.",
