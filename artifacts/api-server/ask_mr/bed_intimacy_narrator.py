"""Bed intimacy engine narrator — templates + fired rules only."""
...
from .bed_intimacy_templates import (
    LEVEL_SCORE_FALLBACK,
    USER_SECTION,
    VERDICT_LABELS,
    detect_bed_intimacy_answer_focus,
...
def _resolve_intim_level(checks: dict[str, Any]) -> str:
    level = str(
        checks.get("intimacy_level")
        or checks.get("bed_intimacy_level")
        or checks.get("intim_level")
        or checks.get("level")
        or ""
    ).strip().lower()
    ...
    for key in ("strained", "sensitive", "harmonious", "mixed", "strong", "moderate"):
        if key in blob:
            if key in ("strong", "moderate"):
                return "harmonious" if key == "strong" else "mixed"
            return key
    return "mixed"
...
def _build_reason_summary(strongest: list[str], weakest: list[str], verdict: str) -> str:
    ...
    if n_pos and n_neg:
        return f"Chart analysis me {n_pos} supportive aur {n_neg} challenging intimacy indicators mile. Isi wajah se final verdict {verdict} hai."
    if n_neg:
        return f"Chart me challenging intimacy indicators zyada active hain. Isi wajah se final verdict {verdict} hai."
    if n_pos:
        return f"Chart me supportive intimacy indicators zyada dikhte hain. Isi wajah se final verdict {verdict} hai."
    return f"Chart ke signals limited ya mixed hain — isi wajah se final verdict {verdict} hai."


def render_bed_intimacy_template_answer(data: dict[str, Any], question: str = "", *, lang: str = "hn") -> str:
    verdict = str(data.get("final_verdict") or "Mixed")
    level = str(data.get("intimacy_level") or data.get("bed_intimacy_level") or verdict).strip().lower()
    angle = str(data.get("answer_focus") or data.get("bed_intimacy_angle") or "general_intimacy")
    ...
    parts = [
        ...
        _join_effect_lines(strongest, prefix=USER_SECTION["positive"], fallback=f"{USER_SECTION['positive']} supportive intimacy indicators limited hain."),
        _join_effect_lines(weakest, prefix=USER_SECTION["challenges"], fallback=f"{USER_SECTION['challenges']} challenging intimacy indicators active hain."),
        f"{USER_SECTION['meaning']} {str(data.get('meaning_note') or '').strip() or get_meaning(angle, level)}",
        f"{USER_SECTION['outlook']} {str(data.get('intim_outlook') or '').strip() or get_intim_outlook(level)}",
...
def validate_bed_intimacy_narrator_output(text: str, data: dict[str, Any]) -> tuple[bool, list[str]]:
    issues: list[str] = []
    t = (text or "").strip()
    if not t:
        return False, ["empty"]
    level = str(data.get("intimacy_level") or data.get("bed_intimacy_level") or "").strip().lower()
    if level == "strained" and re.search(r"(?i)\b(mostly\s+harmonious|comfort\s+align|deepen\s+hoti\s+hai)\b", t):
        issues.append("contradiction_strained")
    if level == "harmonious" and re.search(r"(?i)\b(strained\s+range|pressure\s+avoid|safety\s+first)\b", t):
        issues.append("contradiction_harmonious")
...
def engine_result_to_bed_intimacy_json(
    result: EngineResult,
    question: str = "",
    *,
    ...
    angle = detect_bed_intimacy_answer_focus(q, question_dna=question_dna)
    level = _resolve_intim_level(checks)
    checks.update({
        "bed_intimacy_angle": angle,
        "question": q,
    ...
    payload: dict[str, Any] = {
        ...
        "intim_outlook": get_intim_outlook(level),
    ...
    payload["locked_template"] = render_bed_intimacy_template_answer(payload, question=q)
    return payload


def bed_intimacy_engine_narrator_payload(
    result: EngineResult,
    *,
    ...
    data = engine_result_to_bed_intimacy_json(result, question=q, question_dna=question_dna)
    ...
    return "\n".join([
        ...
        "RULES: Use direct_answer verbatim. Never contradict strained verdict with 'mostly harmonious'.",
