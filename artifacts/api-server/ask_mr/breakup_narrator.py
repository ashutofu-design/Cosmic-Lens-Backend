"""Breakup / separation risk engine narrator — templates + fired rules only."""
...
_BANNED_NARRATOR_PHRASES = re.compile(
    ...
    r"pakka\s+breakup|definite\s+breakup|tootega\s+hi|rishta\s+tootega\s+hi|"
    r"separation\s+pakka|breakup\s+pakka"
    r")\b",
)

_FATALISTIC_PHRASES = re.compile(
    r"(?i)\b(tootega\s+hi|pakka\s+hai|100%|guaranteed|definitely\s+break|will\s+definitely)\b",
)
...
def _resolve_breakup_level(checks: dict[str, Any]) -> str:
    level = str(
        checks.get("risk_level")
        or checks.get("breakup_level")
        or checks.get("level")
        or ""
    ).strip().lower()
    ...
    verdict = str(checks.get("verdict") or "").lower()
    blob = f"{headline} {verdict}"
    for key in ("high", "elevated", "moderate", "low"):
        if key in blob:
            return key
    return "moderate"
...
def _build_reason_summary(strongest: list[str], weakest: list[str], verdict: str) -> str:
    ...
    if n_pos and n_neg:
        return (
            f"Chart analysis me {n_pos} repair-supportive aur {n_neg} separation-risk indicators mile. "
            f"Isi wajah se final verdict {verdict} hai."
        )
    if n_neg:
        return (
            f"Chart me separation-risk indicators zyada active hain. "
            f"Isi wajah se final verdict {verdict} hai."
        )
    if n_pos:
        return (
            f"Chart me repair-supportive indicators zyada dikhte hain. "
            f"Isi wajah se final verdict {verdict} hai."
        )
    return f"Chart ke signals limited ya mixed hain — isi wajah se final verdict {verdict} hai."


def render_breakup_template_answer(
    data: dict[str, Any],
    question: str = "",
    *,
    lang: str = "hn",
) -> str:
    verdict = str(data.get("final_verdict") or data.get("verdict") or "Moderate risk")
    level = str(data.get("breakup_level") or data.get("risk_level") or verdict).strip().lower()
    if level.endswith(" risk"):
        level = level.replace(" risk", "").strip()
    angle = str(data.get("answer_focus") or data.get("breakup_angle") or "general_breakup_risk")
    ...
    p3 = _join_effect_lines(
        ...
        fallback=f"{USER_SECTION['positive']} repair-supportive indicators limited hain.",
    )
    p4 = _join_effect_lines(
        weakest,
        prefix=USER_SECTION["challenges"],
        fallback=f"{USER_SECTION['challenges']} separation-risk indicators active hain.",
    )
    meaning = str(data.get("meaning_note") or "").strip() or get_meaning(angle, level)
    repair = str(data.get("repair_outlook") or "").strip() or get_repair_outlook(level)
...
def validate_breakup_narrator_output(text: str, data: dict[str, Any]) -> tuple[bool, list[str]]:
    issues: list[str] = []
    t = (text or "").strip()
    if not t:
        return False, ["empty"]
    level = str(data.get("breakup_level") or data.get("risk_level") or "").strip().lower()
    if level.endswith(" risk"):
        level = level.replace(" risk", "").strip()
    angle = str(data.get("answer_focus") or "").strip().lower()

    if level == "high":
        if re.search(r"(?i)\b(low\s+risk|no\s+breakup|bond\s+strong|survive\s+easily)\b", t):
            issues.append("contradiction_high_risk")
    if level == "low" and angle == "will_breakup":
        if re.search(r"(?i)\b(high-risk|pakka|tootega\s+hi|definite)\b", t):
            issues.append("contradiction_low_risk")
    if _FATALISTIC_PHRASES.search(t):
        issues.append("fatalistic_phrase")
...
def engine_result_to_breakup_json(
    result: EngineResult,
    question: str = "",
    *,
    ...
    angle = detect_breakup_answer_focus(q, question_dna=question_dna)
    level = _resolve_breakup_level(checks)
    checks.update({"breakup_angle": angle, "question": q, "risk_level": level, "breakup_level": level})

    verdict_label = VERDICT_LABELS.get(level, level.title() if level else "Moderate risk")
    ...
    payload: dict[str, Any] = {
        ...
        "repair_outlook": get_repair_outlook(level),
        ...
        "reconnection_yoga": bool(checks.get("reconnection_yoga")),
        "separation_yoga": bool(checks.get("separation_yoga")),
    }
    payload["_checks"] = checks
    payload["locked_template"] = render_breakup_template_answer(payload, question=q)
    return payload


def breakup_narrator_payload(
    result: EngineResult,
    *,
    ...
    data = engine_result_to_breakup_json(result, question=q, question_dna=question_dna)
    ...
    return "\n".join([
        ...
        locked,
        "",
        "RULES: Use direct_answer verbatim. Never fatalistic (pakka breakup / tootega hi).",
        "HIGH risk = high-risk indicators, NOT guaranteed breakup.",
        "BANNED: clarity, patience, emotional attachment, The Big Picture, pakka breakup.",
    ])
