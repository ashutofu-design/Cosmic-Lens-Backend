"""Chemistry engine narrator — templates + fired rules only."""
...
from .chemistry_templates import (
    LEVEL_SCORE_FALLBACK,
    USER_SECTION,
    VERDICT_LABELS,
    detect_chemistry_answer_focus,
...
def _resolve_chem_level(checks: dict[str, Any]) -> str:
    level = str(
        checks.get("chemistry_level")
        or checks.get("chem_level")
        or checks.get("level")
        or ""
    ).strip().lower()
    ...
    for key in ("strong", "moderate", "uneven", "low", "very strong", "mixed"):
        if key in blob:
            if key == "very strong":
                return "strong"
            if key == "mixed":
...
def _resolve_confidence(level: str, checks: dict[str, Any]) -> tuple[int, str]:
    ...
    try:
        ...
        score = 0
    if score <= 0:
        score = LEVEL_SCORE_FALLBACK.get(level, 50)
...
def _build_reason_summary(strongest: list[str], weakest: list[str], verdict: str) -> str:
    ...
    if n_pos and n_neg:
        return f"Chart analysis me {n_pos} supportive aur {n_neg} challenging chemistry indicators mile. Isi wajah se final verdict {verdict} hai."
    if n_neg:
        return f"Chart me challenging chemistry indicators zyada active hain. Isi wajah se final verdict {verdict} hai."
    if n_pos:
        return f"Chart me supportive chemistry indicators zyada dikhte hain. Isi wajah se final verdict {verdict} hai."
    return f"Chart ke signals limited ya mixed hain — isi wajah se final verdict {verdict} hai."


def render_chemistry_template_answer(data: dict[str, Any], question: str = "", *, lang: str = "hn") -> str:
    verdict = str(data.get("final_verdict") or "Moderate")
    level = str(data.get("chemistry_level") or data.get("chem_level") or verdict).strip().lower()
    angle = str(data.get("answer_focus") or data.get("chemistry_angle") or "general_chemistry")
    ...
    parts = [
        ...
        _join_effect_lines(strongest, prefix=USER_SECTION["positive"], fallback=f"{USER_SECTION['positive']} supportive chemistry indicators limited hain."),
        _join_effect_lines(weakest, prefix=USER_SECTION["challenges"], fallback=f"{USER_SECTION['challenges']} challenging chemistry indicators active hain."),
        f"{USER_SECTION['meaning']} {str(data.get('meaning_note') or '').strip() or get_meaning(angle, level)}",
        f"{USER_SECTION['outlook']} {str(data.get('chem_outlook') or '').strip() or get_chem_outlook(level)}",
...
def validate_chemistry_narrator_output(text: str, data: dict[str, Any]) -> tuple[bool, list[str]]:
    issues: list[str] = []
    t = (text or "").strip()
    if not t:
        return False, ["empty"]
    level = str(data.get("chemistry_level") or data.get("chem_level") or "").strip().lower()
    if level == "low" and re.search(r"(?i)\b(mostly\s+strong|spark\s+active|visible\s+mutual\s+pull)\b", t):
        issues.append("contradiction_low")
    if level == "strong" and re.search(r"(?i)\b(low\s+range|pull\s+weak|spark\s+low)\b", t):
...
def engine_result_to_chemistry_json(
    result: EngineResult,
    question: str = "",
    *,
    ...
    angle = detect_chemistry_answer_focus(q, question_dna=question_dna)
    level = _resolve_chem_level(checks)
    checks.update({
        "chemistry_angle": angle,
        "question": q,
    ...
    payload: dict[str, Any] = {
        ...
        "chem_outlook": get_chem_outlook(level),
    ...
    payload["locked_template"] = render_chemistry_template_answer(payload, question=q)
    return payload


def chemistry_engine_narrator_payload(
    result: EngineResult,
    *,
    ...
    data = engine_result_to_chemistry_json(result, question=q, question_dna=question_dna)
    ...
    return "\n".join([
        ...
        "RULES: Use direct_answer verbatim. Never contradict low verdict with 'mostly strong spark'.",
