"""Communication engine narrator — templates + fired rules only."""
...
from .communication_templates import (
    ...
    USER_SECTION,
    VERDICT_LABELS,
    detect_communication_answer_focus,
...
def _resolve_comm_level(checks: dict[str, Any]) -> str:
    level = str(
        checks.get("communication_level")
        or checks.get("comm_level")
        or checks.get("level")
        or ""
    ).strip().lower()
    if level in VERDICT_LABELS:
        return level
    if level in ("moderate", "mixed"):
        return "uneven"
    blob = f"{checks.get('headline') or ''} {checks.get('verdict') or ''}".lower()
    for key in ("blocked", "strained", "uneven", "clear"):
        if key in blob:
            return key
    return "uneven"
...
def _build_reason_summary(strongest: list[str], weakest: list[str], verdict: str) -> str:
    ...
    if n_pos and n_neg:
        return f"Chart analysis me {n_pos} supportive aur {n_neg} challenging communication indicators mile. Isi wajah se final verdict {verdict} hai."
    if n_neg:
        return f"Chart me challenging communication indicators zyada active hain. Isi wajah se final verdict {verdict} hai."
    if n_pos:
        return f"Chart me supportive communication indicators zyada dikhte hain. Isi wajah se final verdict {verdict} hai."
    return f"Chart ke signals limited ya mixed hain — isi wajah se final verdict {verdict} hai."


def render_communication_template_answer(data: dict[str, Any], question: str = "", *, lang: str = "hn") -> str:
    verdict = str(data.get("final_verdict") or "Uneven")
    level = str(data.get("communication_level") or data.get("comm_level") or verdict).strip().lower()
    if level in ("moderate", "mixed"):
        level = "uneven"
    angle = str(data.get("answer_focus") or data.get("communication_angle") or "general_communication")
    ...
    parts = [
        ...
        _join_effect_lines(strongest, prefix=USER_SECTION["positive"], fallback=f"{USER_SECTION['positive']} supportive communication indicators limited hain."),
        _join_effect_lines(weakest, prefix=USER_SECTION["challenges"], fallback=f"{USER_SECTION['challenges']} challenging communication indicators active hain."),
        f"{USER_SECTION['meaning']} {str(data.get('meaning_note') or '').strip() or get_meaning(angle, level)}",
        f"{USER_SECTION['outlook']} {str(data.get('comm_outlook') or '').strip() or get_comm_outlook(level)}",
...
def validate_communication_narrator_output(text: str, data: dict[str, Any]) -> tuple[bool, list[str]]:
    issues: list[str] = []
    t = (text or "").strip()
    if not t:
        return False, ["empty"]
    level = str(data.get("communication_level") or data.get("comm_level") or "").strip().lower()
    if level in ("moderate", "mixed"):
        level = "uneven"
    if level == "blocked" and re.search(r"(?i)\b(mostly\s+clear|smooth\s+talk|no\s+friction)\b", t):
        issues.append("contradiction_blocked")
    if level == "clear" and re.search(r"(?i)\b(blocked\s+pattern|muted\s+talk|harsh\s+loops)\b", t):
        issues.append("contradiction_clear")
...
def engine_result_to_communication_json(
    result: EngineResult,
    question: str = "",
    *,
    ...
    angle = detect_communication_answer_focus(q, question_dna=question_dna)
    level = _resolve_comm_level(checks)
    checks.update({"communication_angle": angle, "question": q, "communication_level": level, "comm_level": level})
    ...
    payload: dict[str, Any] = {
        ...
        "comm_outlook": get_comm_outlook(level),
    ...
    payload["locked_template"] = render_communication_template_answer(payload, question=q)
    return payload


def communication_engine_narrator_payload(
    result: EngineResult,
    *,
    ...
    data = engine_result_to_communication_json(result, question=q, question_dna=question_dna)
    ...
    return "\n".join([
        ...
        "RULES: Use direct_answer verbatim. Never contradict blocked verdict with 'mostly clear'.",
