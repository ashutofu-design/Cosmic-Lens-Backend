"""Compatibility engine narrator — templates + fired rules only."""
...
_BANNED_NARRATOR_PHRASES = re.compile(
    ...
    r"perfect\s+match|soul\s*mate|100%\s+compatible"
    r")\b",
)
...
def _resolve_compat_level(checks: dict[str, Any]) -> str:
    level = str(
        checks.get("compat_level")
        or checks.get("compatibility_level")
        or checks.get("level")
        or ""
    ).strip().lower()
    ...
    for key in ("strained", "mixed", "moderate", "supportive"):
        if key in blob:
            return key
    return "moderate"
...
def _build_reason_summary(strongest: list[str], weakest: list[str], verdict: str) -> str:
    ...
    if n_pos and n_neg:
        return (
            f"Chart analysis me {n_pos} compatibility-supportive aur {n_neg} friction indicators mile. "
            f"Isi wajah se final verdict {verdict} hai."
        )
    if n_neg:
        return (
            f"Chart me compatibility-friction indicators zyada active hain. "
            f"Isi wajah se final verdict {verdict} hai."
        )
    if n_pos:
        return (
            f"Chart me compatibility-supportive indicators zyada dikhte hain. "
            f"Isi wajah se final verdict {verdict} hai."
        )
    return f"Chart ke signals limited ya mixed hain — isi wajah se final verdict {verdict} hai."


def render_compatibility_template_answer(
    data: dict[str, Any],
    question: str = "",
    *,
    lang: str = "hn",
) -> str:
    verdict = str(data.get("final_verdict") or data.get("verdict") or "Moderate")
    level = str(data.get("compat_level") or data.get("compatibility_level") or verdict).strip().lower()
    angle = str(data.get("answer_focus") or data.get("compatibility_angle") or "general_compatibility")
    ...
    p3 = _join_effect_lines(
        ...
        fallback=f"{USER_SECTION['positive']} compatibility-supportive indicators limited hain.",
    )
    p4 = _join_effect_lines(
        weakest,
        prefix=USER_SECTION["challenges"],
        fallback=f"{USER_SECTION['challenges']} compatibility-friction indicators active hain.",
    )
    meaning = str(data.get("meaning_note") or "").strip() or get_meaning(angle, level)
    growth = str(data.get("growth_outlook") or "").strip() or get_growth_outlook(level)
...
def validate_compatibility_narrator_output(text: str, data: dict[str, Any]) -> tuple[bool, list[str]]:
    issues: list[str] = []
    t = (text or "").strip()
    if not t:
        return False, ["empty"]
    level = str(data.get("compat_level") or data.get("compatibility_level") or "").strip().lower()

    if level == "strained":
        if re.search(r"(?i)\b(strong\s+match|perfect\s+pair|achhi\s+compat|highly\s+compat|supportive\s+dikhti)\b", t):
            issues.append("contradiction_strained_verdict")
    if level == "supportive":
        if re.search(r"(?i)\b(strained\s+pattern|major\s+friction|incompatible)\b", t):
            issues.append("contradiction_supportive_verdict")
...
def engine_result_to_compatibility_json(
    result: EngineResult,
    question: str = "",
    *,
    ...
    angle = detect_compatibility_answer_focus(q, question_dna=question_dna)
    level = _resolve_compat_level(checks)
    checks.update({
        "compatibility_angle": angle,
        "question": q,
        "compat_level": level,
        "compatibility_level": level,
    })

    verdict_label = VERDICT_LABELS.get(level, level.title() if level else "Moderate")
    ...
    payload: dict[str, Any] = {
        ...
        "growth_outlook": get_growth_outlook(level),
    ...
    payload["locked_template"] = render_compatibility_template_answer(payload, question=q)
    return payload


def compatibility_narrator_payload(
    result: EngineResult,
    *,
    ...
    data = engine_result_to_compatibility_json(result, question=q, question_dna=question_dna)
    ...
    return "\n".join([
        ...
        locked,
        "",
        "RULES: Use direct_answer verbatim. Never contradict strained verdict with 'perfect match'.",
        "BANNED: clarity, patience, emotional attachment, The Big Picture, perfect match.",
    ])
