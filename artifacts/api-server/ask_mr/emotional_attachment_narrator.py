"""Emotional attachment engine narrator — templates + fired rules only."""
...
from .emotional_attachment_templates import (
    LEVEL_SCORE_FALLBACK,
    USER_SECTION,
    VERDICT_LABELS,
    detect_emotional_attachment_answer_focus,
...
def _resolve_attach_level(checks: dict[str, Any]) -> str:
    level = str(
        checks.get("attachment_level")
        or checks.get("emotional_attachment_level")
        or checks.get("level")
        or ""
    ).strip().lower()
    ...
    for key in ("volatile", "anxious", "mixed", "secure"):
        if key in blob:
            return key
    return "mixed"
...
def _build_reason_summary(strongest: list[str], weakest: list[str], verdict: str) -> str:
    ...
    if n_pos and n_neg:
        return f"Chart analysis me {n_pos} supportive aur {n_neg} challenging bonding indicators mile. Isi wajah se final verdict {verdict} hai."
    if n_neg:
        return f"Chart me challenging bonding indicators zyada active hain. Isi wajah se final verdict {verdict} hai."
    if n_pos:
        return f"Chart me supportive bonding indicators zyada dikhte hain. Isi wajah se final verdict {verdict} hai."
    return f"Chart ke signals limited ya mixed hain — isi wajah se final verdict {verdict} hai."


def render_emotional_attachment_template_answer(data: dict[str, Any], question: str = "", *, lang: str = "hn") -> str:
    verdict = str(data.get("final_verdict") or "Mixed")
    level = str(data.get("attachment_level") or data.get("emotional_attachment_level") or verdict).strip().lower()
    angle = str(data.get("answer_focus") or data.get("attachment_angle") or "general_attachment")
    ...
    parts = [
        ...
        _join_effect_lines(strongest, prefix=USER_SECTION["positive"], fallback=f"{USER_SECTION['positive']} supportive bonding indicators limited hain."),
        _join_effect_lines(weakest, prefix=USER_SECTION["challenges"], fallback=f"{USER_SECTION['challenges']} challenging bonding indicators active hain."),
        f"{USER_SECTION['meaning']} {str(data.get('meaning_note') or '').strip() or get_meaning(angle, level)}",
        f"{USER_SECTION['outlook']} {str(data.get('attach_outlook') or '').strip() or get_attach_outlook(level)}",
...
def validate_emotional_attachment_narrator_output(text: str, data: dict[str, Any]) -> tuple[bool, list[str]]:
    issues: list[str] = []
    t = (text or "").strip()
    if not t:
        return False, ["empty"]
    level = str(data.get("attachment_level") or data.get("emotional_attachment_level") or "").strip().lower()
    if level == "volatile" and re.search(r"(?i)\b(mostly\s+secure|steady\s+closeness|no\s+volatility)\b", t):
        issues.append("contradiction_volatile")
    if level == "secure" and re.search(r"(?i)\b(volatile\s+pattern|rollercoaster|highs\s+and\s+lows)\b", t):
        issues.append("contradiction_secure")
...
def engine_result_to_emotional_attachment_json(
    result: EngineResult,
    question: str = "",
    *,
    ...
    angle = detect_emotional_attachment_answer_focus(q, question_dna=question_dna)
    level = _resolve_attach_level(checks)
    checks.update({
    ...
    payload: dict[str, Any] = {
        "question_type": "emotional_attachment",
        "original_question": q,
        "attachment_angle": angle,
        "answer_focus": angle,
        "attachment_level": level,
        ...
        "attach_outlook": get_attach_outlook(level),
    ...
    payload["locked_template"] = render_emotional_attachment_template_answer(payload, question=q)
    return payload


def emotional_attachment_engine_narrator_payload(
    result: EngineResult,
    *,
    ...
    data = engine_result_to_emotional_attachment_json(result, question=q, question_dna=question_dna)
    ...
    return "\n".join([
        ...
        "RULES: Use direct_answer verbatim. Never contradict volatile verdict with 'mostly secure'.",
        "BANNED: clarity, patience, emotional attachment phrase, The Big Picture.",
    ])
