...
def format_engine_rich_plain(
    ...
    *,
    ...
    from ask_intent_fidelity import (
        ...
        infer_relationship_remedies_angle,
    ...
    if arch == "loyalty_trust" or infer_loyalty_angle(q):
        ...
        try:
            ...
            big = str(getattr(result, "verdict", "") or "").strip()
    elif arch == "relationship_decisions" or infer_relationship_decisions_angle(q):
        try:
        ...
        try:
            ...
            big = str(getattr(result, "verdict", "") or "").strip()
    elif arch == "relationship_remedies" or infer_relationship_remedies_angle(q):
        try:
            ...
            data = engine_result_to_relationship_remedies_json(result, question=q)
            big = render_relationship_remedies_template_answer(data, q, lang=lang)
        except Exception:
            big = str(getattr(result, "verdict", "") or "").strip()
...
def narrate_mr_engine_llm(
    ...
    *,
    ...
    if arch == "commitment":
        ...
        _rdec_dna = None
        if isinstance(llm_intent, dict):
            _rdec_dna = llm_intent.get("question_dna")
        narrator_json = engine_result_to_relationship_decisions_json(
            engine_result,
            question=question or "",
        ...
        _rem_dna = None
        if isinstance(llm_intent, dict):
            _rem_dna = llm_intent.get("question_dna")
        narrator_json = engine_result_to_relationship_remedies_json(
            engine_result,
            question=question or "",
    ...
    for attempt in range(2):
        try:
            ...
            if arch == "relationship_decisions" and narrator_json:
                ...
                if not ok:
                    print(
                        f"[engine_narrate] relationship_decisions validation failed {issues} — using locked template",
                        flush=True,
                    )
            ...
            if arch == "relationship_remedies" and narrator_json:
                ...
                if not ok:
                    print(
                        f"[engine_narrate] relationship_remedies validation failed {issues} — using locked template",
                        flush=True,
                    )
