"""Family approval engine narrator — templates + fired rules only."""
...
from .family_approval_templates import (
    LEVEL_SCORE_FALLBACK,
    USER_SECTION,
    VERDICT_LABELS,
    detect_family_approval_answer_focus,
...
def _resolve_family_level(checks: dict[str, Any]) -> str:
    level = str(
        checks.get("family_approval_level")
        or checks.get("approval_level")
        or checks.get("level")
        or ""
    ).strip().lower()
    ...
    for key in ("unlikely", "resistant", "mixed", "supportive"):
        if key in blob:
            return key
    return "mixed"
...
def _build_reason_summary(strongest: list[str], weakest: list[str], verdict: str) -> str:
    ...
    if n_pos and n_neg:
        return f"Chart analysis me {n_pos} supportive aur {n_neg} challenging family-approval indicators mile. Isi wajah se final verdict {verdict} hai."
    if n_neg:
        return f"Chart me challenging family-approval indicators zyada active hain. Isi wajah se final verdict {verdict} hai."
    if n_pos:
        return f"Chart me supportive family-approval indicators zyada dikhte hain. Isi wajah se final verdict {verdict} hai."
    return f"Chart ke signals limited ya mixed hain — isi wajah se final verdict {verdict} hai."


def render_family_approval_template_answer(data: dict[str, Any], question: str = "", *, lang: str = "hn") -> str:
    verdict = str(data.get("final_verdict") or "Mixed")
    level = str(data.get("family_approval_level") or data.get("approval_level") or verdict).strip().lower()
    angle = str(data.get("answer_focus") or data.get("family_approval_angle") or "general_approval")
    ...
    parts = [
        ...
        _join_effect_lines(strongest, prefix=USER_SECTION["positive"], fallback=f"{USER_SECTION['positive']} supportive family-approval indicators limited hain."),
        _join_effect_lines(weakest, prefix=USER_SECTION["challenges"], fallback=f"{USER_SECTION['challenges']} challenging family-approval indicators active hain."),
        f"{USER_SECTION['meaning']} {str(data.get('meaning_note') or '').strip() or get_meaning(angle, level)}",
        f"{USER_SECTION['outlook']} {str(data.get('family_outlook') or '').strip() or get_family_outlook(level)}",
...
def validate_family_approval_narrator_output(text: str, data: dict[str, Any]) -> tuple[bool, list[str]]:
    issues: list[str] = []
    t = (text or "").strip()
    if not t:
        return False, ["empty"]
    level = str(data.get("family_approval_level") or data.get("approval_level") or "").strip().lower()
    if level == "unlikely" and re.search(r"(?i)\b(mostly\s+supportive|elders\s+soften\s+easily|easy\s+approval)\b", t):
        issues.append("contradiction_unlikely")
    if level == "supportive" and re.search(r"(?i)\b(unlikely-soon|strong\s+opposition|intense\s+clash)\b", t):
...
def engine_result_to_family_approval_json(
    result: EngineResult,
    question: str = "",
    *,
    ...
    angle = detect_family_approval_answer_focus(q, question_dna=question_dna)
    level = _resolve_family_level(checks)
    checks.update({
        "family_approval_angle": angle,
        "question": q,
        "family_approval_level": level,
    ...
    payload: dict[str, Any] = {
        ...
        "family_outlook": get_family_outlook(level),
    ...
    payload["locked_template"] = render_family_approval_template_answer(payload, question=q)
    return payload


def family_approval_engine_narrator_payload(
    result: EngineResult,
    *,
    ...
    data = engine_result_to_family_approval_json(result, question=q, question_dna=question_dna)
    ...
    return "\n".join([
        ...
        "RULES: Use direct_answer verbatim. Never contradict unlikely verdict with 'mostly supportive'.",
