"""Karmic marriage engine narrator — templates + fired rules only."""
...
from .karmic_marriage_templates import (
    LEVEL_SCORE_FALLBACK,
    USER_SECTION,
    VERDICT_LABELS,
    detect_karmic_marriage_answer_focus,
...
def _resolve_karm_level(checks: dict[str, Any]) -> str:
    level = str(
        checks.get("karmic_level")
        or checks.get("karmic_marriage_level")
        or checks.get("karm_level")
        or checks.get("level")
        or ""
    ).strip().lower()
    ...
    for key in ("strong", "present", "mixed", "weak"):
        if key in blob:
            return key
    return "mixed"
...
def _build_reason_summary(strongest: list[str], weakest: list[str], verdict: str) -> str:
    ...
    if n_pos and n_neg:
        return f"Chart analysis me {n_pos} supportive aur {n_neg} challenging karmic indicators mile. Isi wajah se final verdict {verdict} hai."
    if n_neg:
        return f"Chart me challenging karmic indicators zyada active hain. Isi wajah se final verdict {verdict} hai."
    if n_pos:
        return f"Chart me supportive karmic indicators zyada dikhte hain. Isi wajah se final verdict {verdict} hai."
    return f"Chart ke signals limited ya mixed hain — isi wajah se final verdict {verdict} hai."


def render_karmic_marriage_template_answer(data: dict[str, Any], question: str = "", *, lang: str = "hn") -> str:
    verdict = str(data.get("final_verdict") or "Mixed")
    level = str(data.get("karmic_level") or data.get("karmic_marriage_level") or verdict).strip().lower()
    angle = str(data.get("answer_focus") or data.get("karmic_angle") or "general_karmic")
    ...
    parts = [
        ...
        _join_effect_lines(strongest, prefix=USER_SECTION["positive"], fallback=f"{USER_SECTION['positive']} supportive karmic indicators limited hain."),
        _join_effect_lines(weakest, prefix=USER_SECTION["challenges"], fallback=f"{USER_SECTION['challenges']} challenging karmic indicators active hain."),
        f"{USER_SECTION['meaning']} {str(data.get('meaning_note') or '').strip() or get_meaning(angle, level)}",
        f"{USER_SECTION['outlook']} {str(data.get('karm_outlook') or '').strip() or get_karm_outlook(level)}",
...
def validate_karmic_marriage_narrator_output(text: str, data: dict[str, Any]) -> tuple[bool, list[str]]:
    issues: list[str] = []
    t = (text or "").strip()
    if not t:
        return False, ["empty"]
    level = str(data.get("karmic_level") or data.get("karmic_marriage_level") or "").strip().lower()
    if level == "weak" and re.search(r"(?i)\b(mostly\s+strong|purpose-driven\s+partnership|deep\s+recognition)\b", t):
        issues.append("contradiction_weak")
    if level == "strong" and re.search(r"(?i)\b(weak\s+signal|weak\s+range|practical\s+only)\b", t):
        issues.append("contradiction_strong")
...
def engine_result_to_karmic_marriage_json(
    result: EngineResult,
    question: str = "",
    *,
    ...
    angle = detect_karmic_marriage_answer_focus(q, question_dna=question_dna)
    level = _resolve_karm_level(checks)
    checks.update({
        "karmic_angle": angle,
        "question": q,
        "karmic_level": level,
    ...
    payload: dict[str, Any] = {
        ...
        "karm_outlook": get_karm_outlook(level),
    ...
    payload["locked_template"] = render_karmic_marriage_template_answer(payload, question=q)
    return payload


def karmic_marriage_engine_narrator_payload(
    result: EngineResult,
    *,
    ...
    data = engine_result_to_karmic_marriage_json(result, question=q, question_dna=question_dna)
    ...
    return "\n".join([
        ...
        "RULES: Use direct_answer verbatim. Never contradict weak verdict with 'mostly strong karmic bond'.",
        "BANNED: clarity, patience, emotional attachment, soul mate, The Big Picture.",
    ])
