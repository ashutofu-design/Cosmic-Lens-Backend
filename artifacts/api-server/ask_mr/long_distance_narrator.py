"""Long distance engine narrator — templates + fired rules only."""
...
from .long_distance_templates import (
    LEVEL_SCORE_FALLBACK,
    USER_SECTION,
    VERDICT_LABELS,
    detect_long_distance_answer_focus,
...
def _resolve_ldist_level(checks: dict[str, Any]) -> str:
    level = str(
        checks.get("long_distance_level")
        or checks.get("ldr_level")
        or checks.get("level")
        or ""
    ).strip().lower()
    ...
    for key in ("strained", "fragile", "mixed", "sustainable"):
        if key in blob:
            return key
    return "mixed"
...
def _build_reason_summary(strongest: list[str], weakest: list[str], verdict: str) -> str:
    ...
    if n_pos and n_neg:
        return f"Chart analysis me {n_pos} supportive aur {n_neg} challenging long-distance indicators mile. Isi wajah se final verdict {verdict} hai."
    if n_neg:
        return f"Chart me challenging long-distance indicators zyada active hain. Isi wajah se final verdict {verdict} hai."
    if n_pos:
        return f"Chart me supportive long-distance indicators zyada dikhte hain. Isi wajah se final verdict {verdict} hai."
    return f"Chart ke signals limited ya mixed hain — isi wajah se final verdict {verdict} hai."


def render_long_distance_template_answer(data: dict[str, Any], question: str = "", *, lang: str = "hn") -> str:
    verdict = str(data.get("final_verdict") or "Mixed")
    level = str(data.get("long_distance_level") or data.get("ldr_level") or verdict).strip().lower()
    angle = str(data.get("answer_focus") or data.get("long_distance_angle") or "general_ldr")
    ...
    parts = [
        ...
        _join_effect_lines(strongest, prefix=USER_SECTION["positive"], fallback=f"{USER_SECTION['positive']} supportive long-distance indicators limited hain."),
        _join_effect_lines(weakest, prefix=USER_SECTION["challenges"], fallback=f"{USER_SECTION['challenges']} challenging long-distance indicators active hain."),
        f"{USER_SECTION['meaning']} {str(data.get('meaning_note') or '').strip() or get_meaning(angle, level)}",
        f"{USER_SECTION['outlook']} {str(data.get('ldist_outlook') or '').strip() or get_ldist_outlook(level)}",
...
def validate_long_distance_narrator_output(text: str, data: dict[str, Any]) -> tuple[bool, list[str]]:
    issues: list[str] = []
    t = (text or "").strip()
    if not t:
        return False, ["empty"]
    level = str(data.get("long_distance_level") or data.get("ldr_level") or "").strip().lower()
    if level == "strained" and re.search(r"(?i)\b(mostly\s+sustainable|easy\s+ldr|no\s+strain)\b", t):
        issues.append("contradiction_strained")
    if level == "sustainable" and re.search(r"(?i)\b(strained\s+pattern|bond\s+weaken|reunion\s+critical)\b", t):
        issues.append("contradiction_sustainable")
...
def engine_result_to_long_distance_json(
    result: EngineResult,
    question: str = "",
    *,
    ...
    angle = detect_long_distance_answer_focus(q, question_dna=question_dna)
    level = _resolve_ldist_level(checks)
    checks.update({
        "long_distance_angle": angle,
        "question": q,
    ...
    payload: dict[str, Any] = {
        ...
        "ldist_outlook": get_ldist_outlook(level),
    ...
    payload["locked_template"] = render_long_distance_template_answer(payload, question=q)
    return payload


def long_distance_engine_narrator_payload(
    result: EngineResult,
    *,
    ...
    data = engine_result_to_long_distance_json(result, question=q, question_dna=question_dna)
    ...
    return "\n".join([
        ...
        "RULES: Use direct_answer verbatim. Never contradict strained verdict with 'mostly sustainable'.",
