"""One-sided love engine narrator — templates + fired rules only."""
...
from .one_sided_love_templates import (
    LEVEL_SCORE_FALLBACK,
    USER_SECTION,
    VERDICT_LABELS,
    detect_one_sided_love_answer_focus,
...
def _resolve_oslove_level(checks: dict[str, Any]) -> str:
    level = str(
        checks.get("one_sided_level")
        or checks.get("oslove_level")
        or checks.get("level")
        or ""
    ).strip().lower()
    ...
    for key in ("unlikely", "one_sided", "one-sided", "unclear", "reciprocal"):
        if key.replace("-", "_") in blob.replace("-", "_") or key in blob:
            if key == "one-sided":
                return "one_sided"
            return key.replace("-", "_") if key == "one-sided" else key
    return "unclear"
...
def _build_reason_summary(strongest: list[str], weakest: list[str], verdict: str) -> str:
    ...
    if n_pos and n_neg:
        return f"Chart analysis me {n_pos} supportive aur {n_neg} challenging reciprocity indicators mile. Isi wajah se final verdict {verdict} hai."
    if n_neg:
        return f"Chart me challenging reciprocity indicators zyada active hain. Isi wajah se final verdict {verdict} hai."
    if n_pos:
        return f"Chart me supportive reciprocity indicators zyada dikhte hain. Isi wajah se final verdict {verdict} hai."
    return f"Chart ke signals limited ya mixed hain — isi wajah se final verdict {verdict} hai."


def render_one_sided_love_template_answer(data: dict[str, Any], question: str = "", *, lang: str = "hn") -> str:
    verdict = str(data.get("final_verdict") or "Unclear")
    level = str(data.get("one_sided_level") or data.get("oslove_level") or verdict).strip().lower()
    angle = str(data.get("answer_focus") or data.get("one_sided_angle") or "general_one_sided")
    ...
    parts = [
        ...
        _join_effect_lines(strongest, prefix=USER_SECTION["positive"], fallback=f"{USER_SECTION['positive']} supportive reciprocity indicators limited hain."),
        _join_effect_lines(weakest, prefix=USER_SECTION["challenges"], fallback=f"{USER_SECTION['challenges']} challenging reciprocity indicators active hain."),
        f"{USER_SECTION['meaning']} {str(data.get('meaning_note') or '').strip() or get_meaning(angle, level)}",
        f"{USER_SECTION['outlook']} {str(data.get('oslove_outlook') or '').strip() or get_oslove_outlook(level)}",
...
def validate_one_sided_love_narrator_output(text: str, data: dict[str, Any]) -> tuple[bool, list[str]]:
    issues: list[str] = []
    t = (text or "").strip()
    if not t:
        return False, ["empty"]
    level = str(data.get("one_sided_level") or data.get("oslove_level") or "").strip().lower()
    if level == "unlikely" and re.search(r"(?i)\b(mostly\s+reciprocal|mutual\s+balance\s+strong|equal\s+effort)\b", t):
        issues.append("contradiction_unlikely")
    if level == "reciprocal" and re.search(r"(?i)\b(one-sided\s+zone|over-invest|unlikely-reciprocal)\b", t):
        issues.append("contradiction_reciprocal")
...
def engine_result_to_one_sided_love_json(
    result: EngineResult,
    question: str = "",
    *,
    ...
    angle = detect_one_sided_love_answer_focus(q, question_dna=question_dna)
    level = _resolve_oslove_level(checks)
    checks.update({
        "one_sided_angle": angle,
        "question": q,
    ...
    payload: dict[str, Any] = {
        ...
        "oslove_outlook": get_oslove_outlook(level),
    ...
    payload["locked_template"] = render_one_sided_love_template_answer(payload, question=q)
    return payload


def one_sided_love_engine_narrator_payload(
    result: EngineResult,
    *,
    ...
    data = engine_result_to_one_sided_love_json(result, question=q, question_dna=question_dna)
    ...
    return "\n".join([
        ...
        "RULES: Use direct_answer verbatim. Never contradict unlikely verdict with 'mostly reciprocal'.",
