"""Partner nature engine narrator — templates + fired rules only."""
...
from .partner_nature_templates import (
    LEVEL_SCORE_FALLBACK,
    USER_SECTION,
    VERDICT_LABELS,
    detect_partner_nature_answer_focus,
...
def _resolve_nature_level(checks: dict[str, Any]) -> str:
    level = str(
        checks.get("nature_level")
        or checks.get("partner_nature_level")
        or checks.get("level")
        or ""
    ).strip().lower()
    ...
    for key in ("challenging", "complex", "mixed", "balanced"):
        if key in blob:
            return key
    return "mixed"
...
def _build_reason_summary(strongest: list[str], weakest: list[str], verdict: str) -> str:
    ...
    if n_pos and n_neg:
        return f"Chart analysis me {n_pos} supportive aur {n_neg} challenging nature indicators mile. Isi wajah se final verdict {verdict} hai."
    if n_neg:
        return f"Chart me challenging nature indicators zyada active hain. Isi wajah se final verdict {verdict} hai."
    if n_pos:
        return f"Chart me supportive nature indicators zyada dikhte hain. Isi wajah se final verdict {verdict} hai."
    return f"Chart ke signals limited ya mixed hain — isi wajah se final verdict {verdict} hai."


def render_partner_nature_template_answer(data: dict[str, Any], question: str = "", *, lang: str = "hn") -> str:
    verdict = str(data.get("final_verdict") or "Mixed")
    level = str(data.get("nature_level") or data.get("partner_nature_level") or verdict).strip().lower()
    angle = str(data.get("answer_focus") or data.get("partner_nature_angle") or "general_nature")
    ...
    parts = [
        ...
        _join_effect_lines(strongest, prefix=USER_SECTION["positive"], fallback=f"{USER_SECTION['positive']} supportive nature indicators limited hain."),
        _join_effect_lines(weakest, prefix=USER_SECTION["challenges"], fallback=f"{USER_SECTION['challenges']} challenging nature indicators active hain."),
...
def validate_partner_nature_narrator_output(text: str, data: dict[str, Any]) -> tuple[bool, list[str]]:
    issues: list[str] = []
    t = (text or "").strip()
    if not t:
        return False, ["empty"]
    level = str(data.get("nature_level") or data.get("partner_nature_level") or "").strip().lower()
    if level == "challenging" and re.search(r"(?i)\b(mostly\s+balanced|steady\s+temperament|perfect\s+partner)\b", t):
        issues.append("contradiction_challenging")
    if level == "balanced" and re.search(r"(?i)\b(challenging\s+patterns\s+active|toxic|dominant\s+side\s+zyada)\b", t):
        issues.append("contradiction_balanced")
...
def engine_result_to_partner_nature_json(
    result: EngineResult,
    question: str = "",
    *,
    ...
    angle = detect_partner_nature_answer_focus(q, question_dna=question_dna)
    level = _resolve_nature_level(checks)
    checks.update({"partner_nature_angle": angle, "question": q, "nature_level": level, "partner_nature_level": level})
    ...
    payload: dict[str, Any] = {
        "question_type": "partner_nature",
        "original_question": q,
        "partner_nature_angle": angle,
        "answer_focus": angle,
        "nature_level": level,
        ...
        "nature_outlook": get_nature_outlook(level),
    ...
    payload["locked_template"] = render_partner_nature_template_answer(payload, question=q)
    return payload


def partner_nature_engine_narrator_payload(
    result: EngineResult,
    *,
    ...
    data = engine_result_to_partner_nature_json(result, question=q, question_dna=question_dna)
    ...
    return "\n".join([
        ...
        "RULES: Use direct_answer verbatim. Never contradict challenging verdict with 'balanced mostly'.",
