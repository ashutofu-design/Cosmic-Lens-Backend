"""Relationship future engine narrator — templates + fired rules only."""
...
from .relationship_future_templates import (
    LEVEL_SCORE_FALLBACK,
    USER_SECTION,
    VERDICT_LABELS,
    detect_relationship_future_answer_focus,
...
def _resolve_rfut_level(checks: dict[str, Any]) -> str:
    level = str(
        checks.get("future_level")
        or checks.get("relationship_future_level")
        or checks.get("rfut_level")
        or checks.get("level")
        or ""
    ).strip().lower()
    if level in VERDICT_LABELS:
        return level
    blob = f"{checks.get('headline') or ''} {checks.get('verdict') or ''} {checks.get('future_outlook') or ''}".lower()
    for key in ("promising", "uncertain", "mixed", "weak", "steady", "strained", "cautious", "moderate"):
        if key in blob:
            if key in ("steady", "moderate"):
                return "mixed"
            if key in ("strained", "cautious"):
                return "uncertain" if key == "cautious" else "weak"
            return key
    return "mixed"
...
def _resolve_confidence(level: str, checks: dict[str, Any]) -> tuple[int, str]:
    ...
    try:
        ...
        score = 0
    if score <= 0:
        score = LEVEL_SCORE_FALLBACK.get(level, 52)
...
def _build_reason_summary(strongest: list[str], weakest: list[str], verdict: str) -> str:
    ...
    if n_pos and n_neg:
        return f"Chart analysis me {n_pos} supportive aur {n_neg} challenging future indicators mile. Isi wajah se final verdict {verdict} hai."
    if n_neg:
        return f"Chart me challenging future indicators zyada active hain. Isi wajah se final verdict {verdict} hai."
    if n_pos:
        return f"Chart me supportive future indicators zyada dikhte hain. Isi wajah se final verdict {verdict} hai."
    return f"Chart ke signals limited ya mixed hain — isi wajah se final verdict {verdict} hai."


def render_relationship_future_template_answer(data: dict[str, Any], question: str = "", *, lang: str = "hn") -> str:
    verdict = str(data.get("final_verdict") or "Mixed")
    level = str(data.get("future_level") or data.get("relationship_future_level") or verdict).strip().lower()
    angle = str(data.get("answer_focus") or data.get("future_angle") or "general_future")
    ...
    parts = [
        ...
        _join_effect_lines(strongest, prefix=USER_SECTION["positive"], fallback=f"{USER_SECTION['positive']} supportive future indicators limited hain."),
        _join_effect_lines(weakest, prefix=USER_SECTION["challenges"], fallback=f"{USER_SECTION['challenges']} challenging future indicators active hain."),
        f"{USER_SECTION['meaning']} {str(data.get('meaning_note') or '').strip() or get_meaning(angle, level)}",
        f"{USER_SECTION['outlook']} {str(data.get('rfut_outlook') or '').strip() or get_rfut_outlook(level)}",
...
def validate_relationship_future_narrator_output(text: str, data: dict[str, Any]) -> tuple[bool, list[str]]:
    issues: list[str] = []
    t = (text or "").strip()
    if not t:
        return False, ["empty"]
    level = str(data.get("future_level") or data.get("relationship_future_level") or "").strip().lower()
    if level == "weak" and re.search(r"(?i)\b(mostly\s+promising|bond\s+deepen|strong\s+room)\b", t):
        issues.append("contradiction_weak")
    if level == "promising" and re.search(r"(?i)\b(weak\s+signal|weaken|stagnate)\b", t):
        issues.append("contradiction_promising")
...
def engine_result_to_relationship_future_json(
    result: EngineResult,
    question: str = "",
    *,
    ...
    angle = detect_relationship_future_answer_focus(q, question_dna=question_dna)
    level = _resolve_rfut_level(checks)
    checks.update({
        "future_angle": angle,
        "question": q,
        "future_level": level,
    ...
    payload: dict[str, Any] = {
        "question_type": "relationship_future",
        "original_question": q,
        "future_angle": angle,
        "answer_focus": angle,
        "future_level": level,
        ...
        "rfut_outlook": get_rfut_outlook(level),
    ...
    payload["locked_template"] = render_relationship_future_template_answer(payload, question=q)
    return payload


def relationship_future_engine_narrator_payload(
    result: EngineResult,
    *,
    ...
    data = engine_result_to_relationship_future_json(result, question=q, question_dna=question_dna)
    ...
    return "\n".join([
        ...
        "RULES: Use direct_answer verbatim. Never contradict weak verdict with 'mostly promising'.",
