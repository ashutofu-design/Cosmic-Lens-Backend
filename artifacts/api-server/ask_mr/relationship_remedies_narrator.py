"""Relationship remedies engine narrator — templates + fired rules only."""
...
from .relationship_remedies_templates import (
    LEVEL_SCORE_FALLBACK,
    USER_SECTION,
    VERDICT_LABELS,
    detect_relationship_remedies_answer_focus,
...
_BANNED_NARRATOR_PHRASES = re.compile(
    ...
    r"perfect\s+partner|soul\s*mate|guaranteed\s+miracle"
    r")\b",
)
...
def _resolve_rem_level(checks: dict[str, Any]) -> str:
    level = str(
        checks.get("remedy_level")
        or checks.get("relationship_remedies_level")
        or checks.get("rem_level")
        or checks.get("remedy_scope")
        or checks.get("level")
        or ""
    ).strip().lower()
    ...
    for key in ("supportive", "cautious", "limited", "moderate"):
        if key in blob:
            return key
    return "moderate"
...
def _build_reason_summary(strongest: list[str], weakest: list[str], verdict: str) -> str:
    ...
    if n_pos and n_neg:
        return f"Chart analysis me {n_pos} supportive aur {n_neg} challenging remedy indicators mile. Isi wajah se final verdict {verdict} hai."
    if n_neg:
        return f"Chart me challenging remedy indicators zyada active hain. Isi wajah se final verdict {verdict} hai."
    if n_pos:
        return f"Chart me supportive remedy indicators zyada dikhte hain. Isi wajah se final verdict {verdict} hai."
    return f"Chart ke signals limited ya mixed hain — isi wajah se final verdict {verdict} hai."


def render_relationship_remedies_template_answer(data: dict[str, Any], question: str = "", *, lang: str = "hn") -> str:
    verdict = str(data.get("final_verdict") or "Moderate")
    level = str(data.get("remedy_level") or data.get("relationship_remedies_level") or verdict).strip().lower()
    angle = str(data.get("answer_focus") or data.get("remedy_angle") or "general_remedy")
    ...
    parts = [
        ...
        _join_effect_lines(strongest, prefix=USER_SECTION["positive"], fallback=f"{USER_SECTION['positive']} supportive remedy indicators limited hain."),
        _join_effect_lines(weakest, prefix=USER_SECTION["challenges"], fallback=f"{USER_SECTION['challenges']} challenging remedy indicators active hain."),
        f"{USER_SECTION['meaning']} {str(data.get('meaning_note') or '').strip() or get_meaning(angle, level)}",
        f"{USER_SECTION['outlook']} {str(data.get('rem_outlook') or '').strip() or get_rem_outlook(level)}",
...
def validate_relationship_remedies_narrator_output(text: str, data: dict[str, Any]) -> tuple[bool, list[str]]:
    issues: list[str] = []
    t = (text or "").strip()
    if not t:
        return False, ["empty"]
    level = str(data.get("remedy_level") or data.get("relationship_remedies_level") or "").strip().lower()
    if level == "limited" and re.search(r"(?i)\b(mostly\s+supportive|gentle\s+upay\s*\+\s*daily\s+habit)\b", t):
        issues.append("contradiction_limited")
    if level == "supportive" and re.search(r"(?i)\b(limited\s+range|toxic\s+dynamics|substitute\s+nahi)\b", t):
        issues.append("contradiction_supportive")
...
def engine_result_to_relationship_remedies_json(
    result: EngineResult,
    question: str = "",
    *,
    ...
    angle = detect_relationship_remedies_answer_focus(q, question_dna=question_dna)
    level = _resolve_rem_level(checks)
    checks.update({
        "remedy_angle": angle,
        "question": q,
        "remedy_level": level,
    ...
    if not strongest:
        for item in (result.evidence or [])[:3]:
            line = _compact_evidence_line(str(item))
            if line and not line.lower().startswith("target pattern"):
    ...
    if not weakest:
        ...
        for item in (result.evidence or [])[:3]:
            line = _compact_evidence_line(str(item))
            if line and ("target pattern" in line.lower() or "friction" in line.lower() or "afflict" in line.lower()):
    ...
    payload: dict[str, Any] = {
        "question_type": "relationship_remedies",
        "original_question": q,
        "remedy_angle": angle,
        "answer_focus": angle,
        "remedy_level": level,
        ...
        "rem_outlook": get_rem_outlook(level),
    ...
    payload["locked_template"] = render_relationship_remedies_template_answer(payload, question=q)
    return payload


def relationship_remedies_engine_narrator_payload(
    result: EngineResult,
    *,
    ...
    data = engine_result_to_relationship_remedies_json(result, question=q, question_dna=question_dna)
    ...
    return "\n".join([
        ...
        "RULES: Use direct_answer verbatim. Never contradict limited verdict with 'mostly supportive'.",
        "BANNED: clarity, patience, emotional attachment, guaranteed miracle.",
    ])
