"""Relationship verification engine narrator — templates + fired rules only."""
...
from .relationship_verification_templates import (
...
def _resolve_rver_level(checks: dict[str, Any]) -> str:
    level = str(
        checks.get("verification_level")
        or checks.get("relationship_verification_level")
        or checks.get("rver_level")
        or checks.get("level")
        or ""
    ).strip().lower()
    ...
    for key in ("consistent", "inconsistent", "unreliable", "mixed"):
        if key in blob:
            return key
    return "mixed"
...
def _build_reason_summary(strongest: list[str], weakest: list[str], verdict: str) -> str:
    ...
    if n_pos and n_neg:
        return f"Chart analysis me {n_pos} supportive aur {n_neg} challenging verification indicators mile. Isi wajah se final verdict {verdict} hai."
    if n_neg:
        return f"Chart me challenging verification indicators zyada active hain. Isi wajah se final verdict {verdict} hai."
    if n_pos:
        return f"Chart me supportive verification indicators zyada dikhte hain. Isi wajah se final verdict {verdict} hai."
    return f"Chart ke signals limited ya mixed hain — isi wajah se final verdict {verdict} hai."


def render_relationship_verification_template_answer(data: dict[str, Any], question: str = "", *, lang: str = "hn") -> str:
    verdict = str(data.get("final_verdict") or "Mixed")
    level = str(data.get("verification_level") or data.get("relationship_verification_level") or verdict).strip().lower()
    angle = str(data.get("answer_focus") or data.get("verification_angle") or "general_verification")
    ...
    parts = [
        ...
        _join_effect_lines(strongest, prefix=USER_SECTION["positive"], fallback=f"{USER_SECTION['positive']} supportive verification indicators limited hain."),
        _join_effect_lines(weakest, prefix=USER_SECTION["challenges"], fallback=f"{USER_SECTION['challenges']} challenging verification indicators active hain."),
        f"{USER_SECTION['meaning']} {str(data.get('meaning_note') or '').strip() or get_meaning(angle, level)}",
        f"{USER_SECTION['outlook']} {str(data.get('rver_outlook') or '').strip() or get_rver_outlook(level)}",
...
def validate_relationship_verification_narrator_output(text: str, data: dict[str, Any]) -> tuple[bool, list[str]]:
    issues: list[str] = []
    t = (text or "").strip()
    if not t:
        return False, ["empty"]
    level = str(data.get("verification_level") or data.get("relationship_verification_level") or "").strip().lower()
    if level == "unreliable" and re.search(r"(?i)\b(mostly\s+consistent|words\s*\+\s*actions\s+alignment\s+supportive)\b", t):
        issues.append("contradiction_unreliable")
    if level == "consistent" and re.search(r"(?i)\b(unreliable\s+range|blind\s+trust\s+risky|proof\s+over\s+promises)\b", t):
        issues.append("contradiction_consistent")
...
def engine_result_to_relationship_verification_json(
    result: EngineResult,
    question: str = "",
    *,
    ...
    angle = detect_relationship_verification_answer_focus(q, question_dna=question_dna)
    level = _resolve_rver_level(checks)
    checks.update({
        "verification_angle": angle,
        "question": q,
    ...
    if not strongest:
        for item in (result.evidence or [])[:3]:
            line = _compact_evidence_line(str(item))
            if line and ("support" in line.lower() or "align" in line.lower() or "consistent" in line.lower()):
    ...
    if not weakest:
        ...
        for item in (result.evidence or [])[:3]:
            line = _compact_evidence_line(str(item))
            if line and ("hidden" in line.lower() or "risk" in line.lower() or "gap" in line.lower()):
                if line not in weakest:
                    weakest.append(line)
    ...
    payload: dict[str, Any] = {
        "question_type": "relationship_verification",
        "original_question": q,
        "verification_angle": angle,
        "answer_focus": angle,
        "verification_level": level,
        ...
        "rver_outlook": get_rver_outlook(level),
    ...
    payload["locked_template"] = render_relationship_verification_template_answer(payload, question=q)
    return payload


def relationship_verification_engine_narrator_payload(
    result: EngineResult,
    *,
    ...
    data = engine_result_to_relationship_verification_json(result, question=q, question_dna=question_dna)
    ...
    return "\n".join([
        ...
        "RULES: Use direct_answer verbatim. Never contradict unreliable verdict with 'mostly consistent'.",
