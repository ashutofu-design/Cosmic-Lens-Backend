"""Toxicity engine narrator — templates + fired rules only."""
...
from .toxicity_templates import (
    LEVEL_SCORE_FALLBACK,
    USER_SECTION,
    VERDICT_LABELS,
    detect_toxicity_answer_focus,
...
def _resolve_tox_level(checks: dict[str, Any]) -> str:
    level = str(
        checks.get("toxicity_level")
        or checks.get("tox_level")
        or checks.get("level")
        or ""
    ).strip().lower()
    if level == "watch":
...
def _build_reason_summary(strongest: list[str], weakest: list[str], verdict: str) -> str:
    ...
    if n_pos and n_neg:
        return f"Chart analysis me {n_pos} supportive aur {n_neg} challenging toxicity indicators mile. Isi wajah se final verdict {verdict} hai."
    if n_neg:
        return f"Chart me challenging toxicity indicators zyada active hain. Isi wajah se final verdict {verdict} hai."
    if n_pos:
        return f"Chart me supportive / low-toxicity indicators zyada dikhte hain. Isi wajah se final verdict {verdict} hai."
    return f"Chart ke signals limited ya mixed hain — isi wajah se final verdict {verdict} hai."


def render_toxicity_template_answer(data: dict[str, Any], question: str = "", *, lang: str = "hn") -> str:
    verdict = str(data.get("final_verdict") or "Moderate")
    level = str(data.get("toxicity_level") or data.get("tox_level") or verdict).strip().lower()
    if level == "watch":
        level = "moderate"
    angle = str(data.get("answer_focus") or data.get("toxicity_angle") or "general_toxicity")
    ...
    parts = [
        ...
        _join_effect_lines(strongest, prefix=USER_SECTION["positive"], fallback=f"{USER_SECTION['positive']} low-toxicity / repair indicators limited hain."),
        _join_effect_lines(weakest, prefix=USER_SECTION["challenges"], fallback=f"{USER_SECTION['challenges']} challenging toxicity indicators active hain."),
        f"{USER_SECTION['meaning']} {str(data.get('meaning_note') or '').strip() or get_meaning(angle, level)}",
        f"{USER_SECTION['outlook']} {str(data.get('tox_outlook') or '').strip() or get_tox_outlook(level)}",
...
def validate_toxicity_narrator_output(text: str, data: dict[str, Any]) -> tuple[bool, list[str]]:
    issues: list[str] = []
    t = (text or "").strip()
    if not t:
        return False, ["empty"]
    level = str(data.get("toxicity_level") or data.get("tox_level") or "").strip().lower()
    if level == "watch":
        level = "moderate"
    if level == "high" and re.search(r"(?i)\b(mostly\s+low|repairable\s+friction|no\s+dominant\s+harm)\b", t):
        issues.append("contradiction_high")
    if level == "low" and re.search(r"(?i)\b(high-risk\s+zone|safety\s+immediate|self-protection\s+first)\b", t):
...
def engine_result_to_toxicity_json(
    result: EngineResult,
    question: str = "",
    *,
    ...
    angle = detect_toxicity_answer_focus(q, question_dna=question_dna)
    level = _resolve_tox_level(checks)
    checks.update({
        "toxicity_angle": angle,
        "question": q,
    ...
    payload: dict[str, Any] = {
        ...
        "tox_outlook": get_tox_outlook(level),
    ...
    payload["locked_template"] = render_toxicity_template_answer(payload, question=q)
    return payload


def toxicity_engine_narrator_payload(
    result: EngineResult,
    *,
    ...
    data = engine_result_to_toxicity_json(result, question=q, question_dna=question_dna)
    ...
    return "\n".join([
        ...
        "RULES: Use direct_answer verbatim. Never contradict high verdict with 'mostly low'.",
