"""LLM judge — semantic match of travel answer vs Question DNA contract."""
...
_QUESTION_TYPE_HINTS: dict[str, str] = {
    "cause": "User asked WHY — explain travel cause from chart, not generic advice.",
    "timing": "User asked WHEN — lead with timing window from dasha_timing_compact if present.",
    "decision": "User asked should I / karu ya nahi — balanced travel guidance.",
    "current_state": "User asked current travel/abroad state — present-read from EE dimensions.",
    "risk": "User asked risk — cautious probability; no guaranteed visa / fixed country.",
    "remedy": "User asked what to do — practical prep (docs/language), not muhurat invent.",
    "prediction": "User asked what will happen — stance from EE; no invented dates/countries.",
    "general": "Answer the travel topic asked; no unrelated health/love/money dump.",
}


def travel_dna_judge_enabled() -> bool:
    return (os.environ.get("ASK_TRAVEL_DNA_JUDGE") or "0").strip() != "0"


def travel_dna_judge_model(default: str = "gpt-4.1-mini") -> str:
    return (os.environ.get("ASK_TRAVEL_DNA_JUDGE_MODEL") or default).strip() or default
...
def build_travel_dna_judge_prompt(
    *,
    question: str,
    answer: str,
    ...
    return f"""You are a strict QA judge for Vedic TRAVEL / ABROAD / VISA answers.
...
- Answer guarantees visa / PR / fixed country name as certainty.
...
def llm_judge_travel_dna_alignment(
    client: Any,
    model: str,
    *,
    question: str,
    ...
    if not travel_dna_judge_enabled():
        return True, [], None, {"enabled": False, "skipped": "ASK_TRAVEL_DNA_JUDGE=0"}
    prompt = build_travel_dna_judge_prompt(
        question=question, answer=answer, contract=contract,
    )
    judge_model = travel_dna_judge_model(model)
    try:
...
def build_travel_dna_judge_display(
    question: str,
    answer: str,
    meta: dict[str, Any],
    ...
    enabled = judge_audit.get(
        "enabled", audit.get("enabled", travel_dna_judge_enabled())
    )
    ...
    judge_version = judge_audit.get("judge") or "travel_dna_v1"
...
def run_travel_llm_with_dna_judge(
    client: Any,
    *,
    model: str,
    ...
    audit: dict[str, Any] = {
        "mode": "dna_judge_only",
        "enabled": travel_dna_judge_enabled(),
        "attempts": 1,
        "passed": True,
        "issues": [],
    }
    ...
    text, guard_meta = guard_travel_answer(question, text, meta)
    ...
    try:
        ...
        audit["selected_blocks"] = build_travel_selected_blocks(
    ...
    if travel_dna_judge_enabled():
        ok_j, j_issues, fix_hint, j_audit = llm_judge_travel_dna_alignment(
            client,
            model,
            question=question,
            answer=text,
        ...
        if needs_proof:
            hint = (fix_hint or "").strip() or (
                ...
                "(planet + house/dignity). Keep answer short; no guaranteed visa."
            )
            ...
            try:
                ...
                if text2:
                    text2, guard_meta2 = guard_travel_answer(question, text2, meta)
                    audit["guard"] = guard_meta2
                    text = text2
                    audit["attempts"] = 2
                    audit["proof_retry"] = True
                    ok2, iss2, hint2, j2 = llm_judge_travel_dna_alignment(
        ...
        audit["dna_judge"] = {"enabled": False, "skipped": "ASK_TRAVEL_DNA_JUDGE=0"}
