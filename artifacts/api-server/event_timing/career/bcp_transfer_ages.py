"""BCP transfer ages — 3rd-lord + 12th-lord placement & aspects (move + place change)."""
...
_BCP_SOURCE_WEIGHTS = {
    "3rd_lord_placement": 5.0,
    "3rd_lord_dual_sign_houses": 3.0,
    "3rd_lord_aspects": 5.0,
    "12th_lord_placement": 4.5,
    "12th_lord_dual_sign_houses": 2.5,
    "12th_lord_aspects": 4.5,
}
...
def compute_bcp_transfer_division(
    planets: List[dict],
    lagna_si: int,
    *,
    division: str = "D1",
) -> Dict[str, Any]:
    third_lord = _house_lord(lagna_si, 3)
    twelfth_lord = _house_lord(lagna_si, 12)
    sources_3, ages_3 = _lord_bcp_sources(
        planets, lagna_si, 3,
        division=division, source_prefix="3rd_lord", label_prefix="3L",
    )
    sources_12, ages_12 = _lord_bcp_sources(
        planets, lagna_si, 12,
        division=division, source_prefix="12th_lord", label_prefix="12L",
    )
    all_ages = sorted(ages_3 | ages_12)
    ...
    return {
        ...
        "third_lord_house": _planet_house(planets, third_lord),
        "twelfth_lord": twelfth_lord,
        "twelfth_lord_house": _planet_house(planets, twelfth_lord),
        "aspect_houses_3l": _asp_rows(sources_3, "3rd_lord_aspects"),
        "aspect_houses_12l": _asp_rows(sources_12, "12th_lord_aspects"),
        "sources": sources_3 + sources_12,
        "all_transfer_ages": all_ages,
        "reasoning_summary": (
            f"BCP-{division}: 3L {third_lord} · 12L {twelfth_lord}; ages {all_ages[:12]}"
        ),
    }
...
def _score_bcp_ages(d1: Dict[str, Any], *, user_age: Optional[int] = None) -> List[Dict[str, Any]]:
    ...
    for src in d1.get("sources") or []:
        ...
        rows = (
            ...
            if str(kind).endswith("_placement")
            else (src.get("houses") or [])
        )
        for row in rows:
    ...
    out = [
        {
            ...
            "is_future": user_age is None or r["age"] >= user_age,
        }
...
def compute_bcp_transfer_ages(
    ...
    d1 = compute_bcp_transfer_division(planets, lagna_si, division="D1")
    all_ages = d1.get("all_transfer_ages") or []
    ...
    return {
        "third_lord": d1["third_lord"],
        "third_lord_house": d1["third_lord_house"],
        "twelfth_lord": d1["twelfth_lord"],
        "twelfth_lord_house": d1["twelfth_lord_house"],
        "aspect_houses_3l": d1.get("aspect_houses_3l"),
...
def format_bcp_transfer_age_list(d1: Dict[str, Any]) -> List[Dict[str, Any]]:
    labels = {
        "3rd_lord_placement": "3L placement",
        "3rd_lord_dual_sign_houses": "3L dual-sign",
        "3rd_lord_aspects": "3L aspect",
    ...
    for src in d1.get("sources") or []:
        ...
        if kind and str(kind).endswith("_placement"):
            rows.append({
                "division": div, "rule": rule,
                "detail": src.get("label"), "ages": src.get("ages") or [],
            })
        else:
            for h in src.get("houses") or []:
                rows.append({
                    "division": div, "rule": rule,
                    "detail": h.get("label"), "ages": h.get("ages") or [],
                })
    return rows
