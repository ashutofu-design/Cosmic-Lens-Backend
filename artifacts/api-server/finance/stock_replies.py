...
_NARRATIVE_INSTRUCTIONS = {
    ...
    "loss_reasons": (
        "User asking 'baar-baar loss kyun ho raha hai'. "
        "CRITICAL: Convert abstract astro signals into CONCRETE "
        "BEHAVIORAL MISTAKES the user is making. Do NOT say 'leak', "
        "'blockage', '8th lord', '12th lord', 'dusthana' — those "
        "are jargon. Translate them into actions.\n\n"
        "MAPPING (use afflictions + sub_flags from facts):\n"
        "  - 12L active in dasha          → 'Wrong timing pe entries — "
        "                                    exit aur entry mismatch'\n"
        "  - 8L on H1/H2/H11 OR Mars/Rahu → 'Over-trading aur frequent "
        "                                    trades — sudden reversals "
        "                                    pakad nahi pa rahe'\n"
        "  - speculation_ok=False         → 'Short-term judgement weak — "
        "                                    quick decisions me galti'\n"
        "  - leak_present + dasha link    → 'Profit book nahi karte, "
        "                                    paisa wapis market me '\n"
        "                                    chala jata hai'\n"
        "  - intraday_warning             → 'Day-trading me impulsive '\n"
        "                                    entries'\n"
        "  - H11 weak                     → 'Targets unrealistic, gain '\n"
        "                                    side support nahi'\n\n"
        "REQUIRED FORMAT (exactly):\n"
        "Line 1: 'Loss ke 3 main reasons (chart se):'\n"
        ...
        "in Hinglish, no jargon>'.\n\n"
        "Pick the 3 strongest signals from the facts. NO planet "
        "names, NO house numbers, NO dignity words."
    ),
    "blockage_check": (
