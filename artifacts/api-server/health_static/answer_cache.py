"""Sqlite-backed answer cache — health_static engine.

Mirror of finance_static/answer_cache.py with separate DB file so
health telemetry + cache do not collide with finance.
...
_CACHE_DB = os.environ.get(
    "HEALTH_STATIC_CACHE_DB",
    os.path.join(os.path.dirname(__file__), "_health_static_cache.sqlite3"),
)
...
def get_cached(cache_key: str) -> Optional[dict]:
    ...
    try:
        ...
        print(f"[health_static.cache] get error: {e}", flush=True)
        return None
...
def put_cached(cache_key: str, reply_text: str,
    ...
    try:
        ...
        print(f"[health_static.cache] put error: {e}", flush=True)
        return False
