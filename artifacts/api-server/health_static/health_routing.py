...
def is_health_question(question: str) -> bool:
    ...
    Guard order:
      1. WARNING patterns (incl. timing/death/diagnosis demands) →
         ALWAYS handled by us so the safe template fires.
      2. Health topic keyword present → we own it.
      3. Otherwise False (caller continues to other engines).
    """
    if not isinstance(question, str) or not question.strip():
        return False
    # WARNING patterns bypass the topic gate (architect Phase 2.8.81 lesson)
    for pat, _key in _WARN_PATTERNS:
        if re.search(pat, question, re.IGNORECASE):
            return True
    return bool(_HEALTH_TOPIC_RX.search(question))
