...
# ─────────────────────────────────────────────────────────────────────
# Phase H2.7.10 — LEAN PACK BUILDER (token-saving topic-aware variant)
# ─────────────────────────────────────────────────────────────────────
# Goal: cut 17k → ~7-10k chars by including only sections relevant to
# the user's question. Mirrors build_full_chart_context() architecture
# (same _HEADER/_FOOTER/_MINIMAL_GUIDANCE wrappers, same defensive
# per-section try/except) so output is drop-in compatible.
#
# TIER 0 (ALWAYS included — ~7,600 chars):
#   • _section_birth_lagna   (header, lagna, ascendant, moon)
#   • _section_grahas        (9 planets — sign/house/degree/naks/dignity)
#   • _section_bhavas        (12 house lords — kahaan baithe)
#   • _section_dasha         (current MD/AD/PD)
#
# TIER 1 ADD-ONS (regex on question text):
#   • TIMING keywords  (kab/when/timing/year/saal/month/din/tak)
#       → +_section_future_dasha, +_section_gochar, +_section_dasha_transit_overlay
#   • DASHA-DEEP keywords (next dasha, antardasha breakdown, dasha tree)
#       → +_section_future_dasha (forced)
#   • MARRIAGE/SPOUSE keywords (shaadi/spouse/marriage/biwi/husband/d9/navamsha)
#       → +_section_d9_navamsha
#   • CHILDREN keywords (santaan/putra/d7/saptamsa)
#       → +_section_d9_navamsha (D9 also useful for progeny via 5H)
#   • YOGA/DOSHA keywords (yog/dosh/sade-sati/mangal/raj-yog)
#       → +_section_yogas_doshas
#   • KP keywords (kp/csl/sub-lord/cuspal/ruling-planet)
#       → +_section_kp
#   • DEEP/DETAIL keywords (deep/detail/vistaar/sabhi)
#       → escalate: add D9 + yogas + overlay
#
# Killswitch: callers gate via `LEAN_PACK_MODE=1` env var (handled at
# wire-site in openai_helper.py — passthrough sync + stream).
# ─────────────────────────────────────────────────────────────────────

# Keyword bundles — lowercase, regex word-boundary safe. Hinglish + English.
_LEAN_KW_TIMING = (
    r"\bkab\b|\bwhen\b|\btiming\b|\byear\b|\bsaal\b|\bmonth\b|\bmahin"
    r"|\bdin\b|\bdays?\b|\bweek\b|\btak\b|\bmuhurat|\bdate\b|\bperiod\b"
    r"|\bover\b|\bend\b|\bkhatam|\bstart\b|\bshuru"
)
_LEAN_KW_DASHA = (
    r"\bdasha\b|\bantardasha\b|\bmaha[-\s]?dasha\b|\bvimshottari\b"
    r"|\bpratyantar\b|\bsookshma\b"
)
_LEAN_KW_MARRIAGE = (
    r"\bshaadi\b|\bshadi\b|\bmarri|\bspouse\b|\bbiwi\b|\bpati\b|\bhusband\b"
    r"|\bwife\b|\bpartner\b|\bd9\b|\bnavamsha\b|\bnavamsa\b|\b7th\s*lord\b"
    r"|\bvivah\b|\bsagai\b|\bengagement\b|\brishta\b"
)
_LEAN_KW_CHILD = (
    r"\bsantaan\b|\bsantan\b|\bputra\b|\bbeta\b|\bbeti\b|\bchild\b"
    r"|\bchildren\b|\bd7\b|\bsaptamsa\b|\bsaptamsha\b|\bpregnan|\bgarbh"
)
_LEAN_KW_YOGA = (
    r"\byog\b|\byoga\b|\bdosh\b|\bdosha\b|\bsade[-\s]?sati\b|\bmangal\b"
    r"|\braj[-\s]?yog|\bdhan[-\s]?yog|\bgaja[-\s]?kesari|\bkaal[-\s]?sarp"
)
_LEAN_KW_KP = (
    r"\bkp\b|\bcsl\b|\bsub[-\s]?lord\b|\bcuspal\b|\bruling[-\s]?planet"
    r"|\bcusp\b|\bsignificator\b"
)
_LEAN_KW_DEEP = (
    # Note: \bfull\b removed (H2.7.10 post-review) — too generic, fired on
    # phrases like "full name" / "full kundli batao" without intent for
    # deep multi-section escalation. Replaced with explicit deep-analysis
    # anchors only.
    r"\bdeep\b|\bdetail\b|\bdetails\b|\bvistaar\b|\bvistar\b|\bsabhi\b"
    r"|\bcomplete\s+analy|\banaly[sz]e\b|\banaly[sz]is\b|\bgehra"
    r"|\bin[-\s]?depth\b|\bthorough\b"
)
_LEAN_KW_GOCHAR = (
    r"\bgochar\b|\btransit\b|\bcurrent\s+sky\b|\bjupiter\s+transit"
    r"|\bsaturn\s+transit\b|\baaj\s*kal\b"
)


def _lean_select_tiers(question: str) -> dict:
    """Inspect question text → return dict of which optional sections fire.
    Pure regex, no LLM call. Defensive: empty question → all flags False.
    """
    import re as _re_lean
    q = (question or "").lower()
    if not q.strip():
        return {
            "future_dasha": False, "gochar": False, "overlay": False,
            "d9": False, "yogas": False, "kp": False,
        }
    is_timing = bool(_re_lean.search(_LEAN_KW_TIMING, q))
    is_dasha  = bool(_re_lean.search(_LEAN_KW_DASHA, q))
    is_marri  = bool(_re_lean.search(_LEAN_KW_MARRIAGE, q))
    is_child  = bool(_re_lean.search(_LEAN_KW_CHILD, q))
    is_yoga   = bool(_re_lean.search(_LEAN_KW_YOGA, q))
    is_kp     = bool(_re_lean.search(_LEAN_KW_KP, q))
    is_deep   = bool(_re_lean.search(_LEAN_KW_DEEP, q))
    is_gochar = bool(_re_lean.search(_LEAN_KW_GOCHAR, q))
    return {
        "future_dasha": is_timing or is_dasha or is_deep,
        "gochar":       is_timing or is_gochar or is_deep,
        "overlay":      is_timing or is_deep,
        "d9":           is_marri or is_child or is_deep,
        "yogas":        is_yoga or is_deep,
        "kp":           is_kp or is_deep,
    }


def build_lean_chart_context(
    kundli: Any,
    intel: Any = None,
    birth: Any = None,
    question: str = "",
) -> str:
    """Token-lean variant of build_full_chart_context().

    Always includes Tier 0 (lagna+grahas+bhavas+current dasha).
    Conditionally adds D9 / yogas / KP / future-dasha / gochar / overlay
    based on regex keywords in `question`.

    Returns "" when kundli missing — caller no-ops.
    Never raises; per-section try/except mirrors full builder.

    Security: `question` is used ONLY for tier selection (regex on
    lowercase copy). NOT echoed into the system block — same contract
    as build_full_chart_context().
    """
    if not isinstance(kundli, dict) or not kundli:
        return ""
    ...
    tiers = _lean_select_tiers(question)
    sections: list[str] = []
...
def _lean_tier_summary(question: str) -> dict:
    """Telemetry helper — exposes selected tiers for _trace() logging.
    Returned dict is JSON-safe.
    """
    t = _lean_select_tiers(question)
    return {
        "tier0": "lagna+grahas+bhavas+dasha",
        "future_dasha": t["future_dasha"],
        "gochar": t["gochar"],
        "overlay": t["overlay"],
        "d9": t["d9"],
        "yogas": t["yogas"],
