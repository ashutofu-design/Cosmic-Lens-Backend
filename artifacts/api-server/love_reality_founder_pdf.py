"""Founder-authored Love Reality Pro PDF — Cosmic Lens Universal Pro design."""
...
def _build_story(
    *,
    ...
    report_title = founder_report_title(render_lang)

    buf = io.BytesIO()
    m = pro.content_margins()
    doc = SimpleDocTemplate(buf, pagesize=A4, **m)
    doc.milan_pdf_lang = render_lang
    doc.pro_footer_left = "Cosmic Lens · Powered by Cosmic Intelligence Engine"
    ...
    for i, page_body in enumerate(page_list):
        ...
        flow = _flowable_image(
            img_list[i] if i < len(img_list) else None, max_w=max_w, max_h=max_h
        )
        if flow is not None:
            story.append(flow)
            story.append(Spacer(1, 5 * mm))
        for block in iter_verbatim_blocks(page_body):
            plain = _sanitize_founder_plain(_latinize_pdf_plain(block.text, render_lang))
            if not plain:
                continue
            if _has_indic(plain):
                st = _pick_body_premium(plain, base, render_lang, relax=True)
                st = ParagraphStyle(
                    f"lr_pro_indic_{id(plain)}",
                    parent=st,
                    textColor=pro.TEXT,
                    alignment=TA_LEFT,
                    spaceAfter=10,
