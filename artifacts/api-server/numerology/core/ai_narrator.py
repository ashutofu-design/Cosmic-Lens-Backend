...
This narrator wraps GPT-4.1 to turn those FACTS into premium prose in an
elite Indian astrologer voice — observational, varied, grounded, human.
Scores, remedies, dosha/compatibility logic stay in the engine; AI only
explains what the chart already shows.
...
4. If batch AI output fails fact-guard or voice-quality, a depth-regen retry
   pass re-calls the model individually with a stricter rewrite prompt
   before falling back to static tier content (Phase 3).
5. 3 languages supported: english, hindi, hinglish.

Model: gpt-4o by default (user-provided OPENAI_API_KEY). Override via
OPENAI_NARRATOR_MODEL=gpt-4.1-mini to roll back to the cheaper tier.
"""
...
_VALIDATORS: Dict[str, Callable[[Dict[str, Any], str], bool]] = {
    ...
    # ── Numerology Pro Phase 2 — AI explanation intros ─────────────
    # These paragraphs sit above engine/static tables. At minimum they must
    # reference the user's locked Driver number; several also require the
    # relevant section-specific number/fact so generic prose is rejected.
    "numpro.life_blueprint":
        lambda f, t: _has_num(t, f.get("driver_number")),
    "numpro.executive_summary":
        lambda f, t: _has_num(t, f.get("driver_number")),
    "numpro.life_essence":
        lambda f, t: _has_num(t, f.get("driver_number")),
    "numpro.career_blueprint":
        lambda f, t: _has_num(t, f.get("driver_number")),
    "numpro.love_pattern":
        lambda f, t: _has_num(t, f.get("driver_number")),
    "numpro.wellness_purpose":
        lambda f, t: _has_num(t, f.get("driver_number")),
    "numpro.risk_alerts":
        lambda f, t: _has_num(t, f.get("driver_number")),
    "numpro.monthly_forecast":
        lambda f, t: _has_num(t, f.get("driver_number"))
                     and _has_num(t, f.get("personal_year")),
    "numpro.deep_compat":
        lambda f, t: _has_num(t, f.get("driver_number")),
    "numpro.lucky_numbers":
        lambda f, t: _has_num(t, f.get("driver_number")),
    "numpro.business_launch":
        lambda f, t: _has_num(t, f.get("driver_number")),
    "numpro.name_correction":
        lambda f, t: _has_num(t, f.get("driver_number")),
    "numpro.mobile_number":
        lambda f, t: _has_num(t, f.get("driver_number"))
                     and _has_num(t, f.get("reduced_number")),
    "numpro.vehicle_number":
        lambda f, t: _has_num(t, f.get("driver_number"))
                     and _has_num(t, f.get("reduced_number")),
    "numpro.house_number":
        lambda f, t: _has_num(t, f.get("driver_number"))
...
# Voice system: vedic.numerology.report_voice (single source of truth)
from numerology.core.report_voice import (
    LANG_INSTRUCT as _LANG_INSTRUCT,
    VOICE_GUIDE as _VOICE_GUIDE,
    enrich_ai_spec,
    enrich_all_specs,
    passes_voice_quality,
    post_process_prose,
    build_single_section_user_suffix,
    opening_style_for_section,
    scale_word_target,
    section_profile,
    writing_mode_for_section,
)
...
def _build_prompt(section_key: str, facts: Dict[str, Any], lang: str,
    ...
    min_words = int(word_target * 0.82)
    max_words = int(word_target * 1.08)
    mode = writing_mode_for_section(section_key)
    opening = opening_style_for_section(section_key)
    suffix = build_single_section_user_suffix(section_key, word_target, mode, opening)

    sys_prompt = _VOICE_GUIDE + f"\n\nLANGUAGE RULE: {lang_rule}\n"
    sys_prompt += (
        f"\nLENGTH RULE: Write between {min_words} and {max_words} words. "
        "Prefer fewer, stronger sentences over padding."
    )

    user_prompt = (
        f"Section: {section_key}\n\n"
        f"FACTS (use ONLY these — do not invent new ones):\n{facts_block}\n\n"
        f"Write the narration for this section in {lang}. {suffix}"
    )
    return sys_prompt, user_prompt


def _default_model() -> str:
    """Default model for AI narration.

    Phase 3 upgrade: default to ``gpt-4o`` (premium model) — same tier as
    Love Reality Pro. The fundamentally smarter prose is what makes the
    output read as "high-end numerologist" instead of "competent chatbot".

    Override per environment via ``OPENAI_NARRATOR_MODEL``. To roll back
    to the cheaper model set ``OPENAI_NARRATOR_MODEL=gpt-4.1-mini``.
    """
    return os.environ.get("OPENAI_NARRATOR_MODEL", "gpt-4o").strip() or "gpt-4o"


def _depth_regen_model() -> str:
    """Model for the depth-regen retry pass.

    For the retry we ALWAYS use ``gpt-4o`` regardless of the primary
    model. The retry happens for a small subset (failed sections only),
    so the cost increase is bounded, and we want the strongest available
    model to produce the human-quality replacement.
    """
    return os.environ.get("OPENAI_NARRATOR_REGEN_MODEL", "gpt-4o").strip() or "gpt-4o"
...
def narrate(section_key: str, facts: Dict[str, Any], lang: str = "hinglish",
    ...
    """
    word_target = scale_word_target(section_key, word_target)
    ...
    try:
        resp = client.chat.completions.create(
            model=model,
            temperature=0.60,  # grounded variation — less theatrical drift
            messages=[
        ...
        if _nc is not None:
            try:
                ...
                _nc.record_spend(_nc.cost_for(in_t, out_t, model))
            except Exception:
                pass

        if not text:
            return None
        text = post_process_prose(text)
        ...
        if not passes_voice_quality(text, section_key):
            print(f"[ai_narrator] {section_key} ({lang}) FAILED VOICE-QUALITY — "
...
def _numerology_group_size(n_specs: int) -> int:
    """Default 6 sections/call; NUMEROLOGY_AI_GROUP_SIZE=0 → one call for all."""
    raw = (os.environ.get("NUMEROLOGY_AI_GROUP_SIZE") or "6").strip().lower()
...
def narrate_batch(specs: list, concurrency: int = 3,
    ...
    per ~6 sections, or one call when NUMEROLOGY_AI_GROUP_SIZE=0).
    Drop-in replacement for the old per-section batcher.
    ...
    gs = _numerology_group_size(len(specs))
    # One mega-batch needs no parallel group workers.
    conc = 1 if gs >= len(specs) else concurrency
    return narrate_grouped_batch(specs, group_size=gs, concurrency=conc,
                                 person_name=person_name, dob=dob)
...
def _build_group_prompt(group: list, lang: str) -> tuple[str, str]:
    ...
    for spec in group:
        ...
        sections_payload.append({
            ...
            "voice_profile": spec.get("voice_profile", "standard"),
            "facts": "\n".join(facts_lines) if facts_lines else "(none)",
            "mode_hint": spec.get("mode_hint", ""),
            "opening_hint": spec.get("opening_hint", ""),
        })
    ...
    sys_prompt = (
        ...
        + "LENGTH RULE: Honour each section's word_target (±12%). Tight, high-impact; "
          "some sections may be only 3–5 sharp lines.\n"
        + "VARIATION RULE (batch): Follow each section's PROFILE/MODE/OPENING exactly. "
          + (
              "NUMEROLOGY-ONLY batch: numbers/name/DOB only — zero kundli/dasha/nakshatra language. "
              if all((s.get("section_key") or "").startswith("tier1.") for s in group)
              else "Most sections = short clinical astrology, NOT emotional monologue. "
          )
          + "Never start any section with a question. Never use signature closers. "
          "ZERO chai/cricket/train/monsoon metaphors.\n"
    )

    sections_text = "\n\n".join(
        f"━━━ [{s['key']}] {s['section_key']} | ~{s['word_target']} words | "
        f"profile={s.get('voice_profile', 'standard')} ━━━\n"
        f"{s['mode_hint']}\n"
        f"{s['opening_hint']}\n"
        f"FACTS:\n{s['facts']}"
    ...
    user_prompt = (
        ...
        + (
            "Tone: elite numerologist — numbers and name vibration only; NOT an astrologer. "
            if all((s.get("section_key") or "").startswith("tier1.") for s in group)
            else "Tone: real intelligent astrologer — NOT spiritual AI. "
        )
        + f"Shorter is better. Blunt where facts are hard.\n\n"
        f"{sections_text}"
    )
    return sys_prompt, user_prompt
...
def _call_grouped(group: list, lang: str, model: str) -> Dict[str, str]:
    ...
    try:
        ...
        for spec in group:
            k = spec["key"]
            txt = post_process_prose((parsed.get(k) or "").strip())
            if not txt:
                continue
            sec_key = spec["section_key"]
            facts = spec.get("facts") or {}
            if not _validate(sec_key, facts, txt):
                print(f"[ai_narrator.grouped] {sec_key} ({lang}) "
                      f"FAILED FACT-GUARD — falling back to static.")
                continue
            if not passes_voice_quality(txt, sec_key):
                print(f"[ai_narrator.grouped] {sec_key} ({lang}) "
                      f"FAILED VOICE-QUALITY — falling back to static.")
                continue
            validated[k] = txt
        return validated
...
def narrate_grouped_batch(specs: list, group_size: int = 6,
    ...
    for spec in specs:
        ...
        if "voice_profile" not in spec:
            enrich_ai_spec(spec, batch_index=len(pending))
        pending.append(spec)
    ...
    # gpt-4o → ~$0.06-0.10 per grouped call (6 sections × 280 words).
    # gpt-4.1-mini → ~$0.005-0.010 per grouped call.
    model = _default_model()
    if nc:
        per_group_est = 0.012  # mini default
        if "mini" not in model.lower():
            per_group_est = 0.075  # gpt-4o generous upper bound
        est_cost = len(groups) * per_group_est
        if est_cost > nc.PER_REPORT_LIMIT_USD:
            max_groups = max(1, int(nc.PER_REPORT_LIMIT_USD / per_group_est))
            print(f"[ai_narrator] per-report cap ${nc.PER_REPORT_LIMIT_USD}: "
                  f"trimming {len(groups)} → {max_groups} groups "
                  f"(model={model}, est ${per_group_est}/group); "
    ...
    # ── Step 6: Depth-regen retry pass (Phase 3) ────────────────────────
    # For any spec that the batch call did NOT successfully fill, retry
    # individually with the strongest available model and a stricter
    # "rewrite" prompt that calls out the failure mode explicitly.
    #
    # This matches the Love Reality Pro pipeline — when a chapter comes
    # back too shallow / too short / fails voice-quality, we re-call AI
    # rather than silently falling back to static.
    failed_specs = [sp for sp in pending if not results.get(sp["key"])]
    if failed_specs:
        regen_results = _retry_failed_specs(
            failed_specs,
            person_name=person_name,
            dob=dob,
            max_retries=int(os.environ.get("NARRATOR_REGEN_MAX_RETRIES", "1") or "1"),
            nc=nc,
        )
        results.update(regen_results)
...
def _retry_failed_specs(
    ...
    *,
    ...
    """Phase 3 depth-regen: re-call AI for failed specs with a stricter prompt.

    Failed = batch call returned empty/missing, or fact-guard/voice-quality
    rejected the first attempt. We retry one-at-a-time (no batching) using
    the strongest available model so each retry has full attention budget.

    Never raises. Returns {key: text} for any retry that succeeded; missing
    keys mean the retry also failed and caller should use static fallback.
    """
    out: Dict[str, str] = {}
    if not specs:
        return out
    ...
    regen_model = _depth_regen_model()
    print(f"[ai_narrator.regen] retrying {len(specs)} failed section(s) "
          f"with model={regen_model} (max_retries={max_retries})")
    ...
    for spec in specs:
        ...
        for attempt in range(max(1, max_retries) + 1):
            try:
                text = _call_single_regen(
                    sec_key, facts, lang, word_target, regen_model, attempt
                )
            except Exception as exc:
                print(f"[ai_narrator.regen] {sec_key} attempt {attempt + 1} "
                      f"raised: {exc}")
                text = ""
            if not text:
                continue
            text = post_process_prose(text)
            if not _validate(sec_key, facts, text):
                print(f"[ai_narrator.regen] {sec_key} attempt {attempt + 1} "
                      f"FAILED FACT-GUARD")
                continue
            if not passes_voice_quality(text, sec_key):
                print(f"[ai_narrator.regen] {sec_key} attempt {attempt + 1} "
                      f"FAILED VOICE-QUALITY")
                continue
            break  # accepted

        if text and _validate(sec_key, facts, text) \
                and passes_voice_quality(text, sec_key):
            out[key] = text
            if nc and person_name and dob:
                try:
                    nc.put(person_name, dob, lang, sec_key, facts, text)
                except Exception:
                    pass
            print(f"[ai_narrator.regen] {sec_key} ({lang}) ✓ accepted "
                  f"({len(text.split())} words)")
        else:
            print(f"[ai_narrator.regen] {sec_key} ({lang}) ✗ all retries "
                  f"failed — caller will use static fallback")

    return out


def _call_single_regen(
    section_key: str,
    facts: Dict[str, Any],
    lang: str,
    word_target: int,
    ...
    """One stricter individual call for the regen pass.

    Strategy differs by attempt index:
      attempt 0 → "first regen": stronger system prompt + explicit failure callout
      attempt 1+ → "last chance": shorten target, demand bluntness, hard cap on length
    """
    client = _get_client()
    if client is None:
        return ""
    ...
    mode = writing_mode_for_section(section_key)
    opening = opening_style_for_section(section_key, batch_index=attempt + 7)
    suffix = build_single_section_user_suffix(section_key, word_target, mode, opening)
    ...
    lang_rule = _LANG_INSTRUCT.get(lang, _LANG_INSTRUCT["hinglish"])
    failure_note = (
        "\n\nIMPORTANT — this is a REWRITE because the previous attempt was "
        "rejected for being too generic, too short, or sounding AI-written. "
        "Be more specific to THIS person's facts. Lead with a concrete "
        "behavioural observation, not an abstract trait. Match the rhythm "
        "of the few-shot examples above. No generic motivation. No closing "
        "blessings."
    )
    if attempt >= 1:
        failure_note += (
            "\n\nFINAL ATTEMPT — be even tighter. Cut all hedging. Use one "
            "blunt 5-word line plus two precise observation lines. Quality "
            "over quantity."
        )
    ...
    sys_prompt += (
        f"\nLENGTH RULE: Write between {min_words} and {max_words} words. "
        "Prefer fewer, sharper sentences over padding."
    ) + failure_note
    ...
    capped_max = min(MAX_TOKENS_PER_CALL, max(160, int(word_target * 1.8)))

    try:
        resp = client.chat.completions.create(
            model=model,
            temperature=0.55 if attempt == 0 else 0.40,  # tighter on last try
            messages=[
        ...
        print(f"[ai_narrator.regen] {section_key} call failed: {exc}")
        return ""
