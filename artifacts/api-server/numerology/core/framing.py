...
Usage from a tier renderer:
    from numerology.core.framing import (
...
def _deva_safe(text: str) -> str:
    """Wrap Devanagari runs in the registered Noto alias (if available)."""
    if not text:
        return text
    try:
        ...
        alias = pair[0] if pair else None
    except Exception:
        alias = None
    if not alias:
        return str(text)
    return _DEVA_RE.sub(rf"<font name='{alias}'>\1</font>", str(text))

# ── Number archetypes (pure numerology) ────────────────────────────────
from numerology.core.digits import ARCHETYPE_BY_DRIVER, number_meaning_for
from numerology.core.toolkit import LUCKY
from numerology.core.sanitize import sanitize_text

ARCHETYPE_BY_DRIVER = ARCHETYPE_BY_DRIVER  # re-export
PLANET_BY_DRIVER = {n: number_meaning_for(n) for n in range(1, 10)}  # legacy alias
PLANET_HI: Dict[str, str] = {}  # deprecated — use archetype strings
...
for _area in LIFE_AREA.values():
    for _k, _v in list(_area.items()):
        _area[_k] = sanitize_text(_v)
...
def _T(lang: str, en: str, hi: str, hg: str) -> str:
    if lang == "english":
        return sanitize_text(en)
    if lang == "hindi":
        return sanitize_text(hi)
    return sanitize_text(hg)
...
def numerology_opener_block(s: Dict[str, Any], driver: int, conductor: int,
    ...
    synergy_label = {
        ...
        "SYNERGY":  _T(lang, "SYNERGY (numbers share a supportive pattern)",
                       "तालमेल (संख्याओं का सहायक पैटर्न)",
                       "SYNERGY (numbers share supportive pattern)"),
        "FRICTION": _T(lang, "FRICTION (behavioral mismatch — extra patience needed)",
                       "घर्षण (व्यवहारिक असंगति — अतिरिक्त धैर्य ज़रूरी)",
                       "FRICTION (behavioral mismatch — extra patience needed)"),
        "NEUTRAL":  _T(lang, "NEUTRAL (balanced number dynamic)",
                       "तटस्थ (संतुलित अंक-गतिशीलता)",
                       "NEUTRAL (balanced number dynamic)"),
    }[synergy]
...
def numerology_closing_toolkit_block(s: Dict[str, Any], driver: int, conductor: int,
    ...
    rows = [
        ...
        [_T(lang, "Daily affirmation", "दैनिक अभिव्यक्ति", "Daily affirmation"),
         lk.get("affirmation", "—")],
        [_T(lang, "Daily practice", "दैनिक अभ्यास", "Daily practice"),
         lk.get("practice", "—")],
    ...
    closer_html = _T(lang,
        ...
        f"key meetings, repeat your affirmation on those days, and avoid scheduling on "
        ...
        f"affirmation practice karein, aur AVOID-numbers par scheduling se bachein (specially jab "
