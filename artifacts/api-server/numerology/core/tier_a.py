...
def analyze_number_string(value: str, *, kind: str,
    ...
    """Delegate to psychometric asset engine (numerology.core.asset_analysis)."""
    from numerology.core.asset_analysis import analyze_number_string as _analyze
    return _analyze(value, kind=kind, driver=driver, conductor=conductor)
...
def karmic_lessons(name: str) -> Dict[str, Any]:
    ...
    LESSONS = {
        ...
        7: ("Inner depth — trust intuition, build reflective discipline, "
    ...
    return {
        ...
        "summary": (
            ...
            "Inhe consciously develop karne se rapid personal growth hota hai."
            if missing else
...
def maturity_number(life_path: int, expression: int) -> Dict[str, Any]:
    ...
    archetype = ARCHETYPE_BY_NUMBER.get(_reduce(mat), "—")
    ...
    MATURITY = {
        ...
        7: "Late-life wisdom — research, reflection, retreat, and deep analysis.",
        8: "Late-life material mastery — wealth, authority, recognition.",
        9: "Late-life humanitarian — philanthropy, completion, global vision.",
        11: "Late-life inspiration — intuitive teaching, mentoring at scale.",
    ...
    return {
        ...
        "maturity": mat,
        "archetype": archetype,
        "planet": archetype,  # legacy alias — same as archetype (psychology label)
...
def chaldean_name_numbers(name: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "note": ("Chaldean is the older, stricter system used by Cheiro. "
                 "Use Pythagorean for personality analysis, Chaldean for "
...
def digit_breakdown(value: str) -> List[Dict[str, Any]]:
    """Break a number string into per-digit list with psychology meaning."""
    DIGIT_MEANING = {
        0: "Amplifier — intensifies the digit beside it.",
        1: "Leadership, initiative, visibility.",
        2: "Cooperation, empathy, listening.",
        3: "Creativity, messaging, teaching.",
        4: "Structure, systems, steady work.",
        5: "Change, networking, agility.",
        6: "Harmony, service, aesthetics.",
        7: "Analysis, privacy, depth.",
        8: "Power, accountability, scale.",
        9: "Completion, intensity, impact.",
    }
    out = []
    for d in _digits_of(value):
        arch = ARCHETYPE_BY_NUMBER.get(d, "—")
        out.append({
            "digit": d,
            "archetype": arch,
            "planet": arch,  # legacy
            "meaning": DIGIT_MEANING.get(d, ""),
        })
    return out


def repeating_digit_alerts(value: str) -> List[str]:
    """Detect concerning patterns from repeating digits."""
    ...
    if counts.get(4, 0) >= 3:
        alerts.append("⚠ Digit 4 repeats often — rigidity, rework loops, "
                      "unexpected process changes.")
    if counts.get(8, 0) >= 3:
        alerts.append("⚠ Digit 8 repeats often — pressure cycles; "
                      "strong results with disciplined habits.")
    if counts.get(0, 0) >= 3:
        alerts.append("⚠ Multiple 0s — amplifies neighbours; "
                      "pattern can feel unstable.")
    if total >= 4 and counts.get(7, 0) >= 3:
        alerts.append("ℹ Digit 7 repeated — deep focus; "
                      "may reduce social responsiveness.")
...
def compatibility_matrix(driver: int) -> List[Dict[str, Any]]:
    ...
    for n in range(1, 10):
        ...
        out.append({
            "number": n,
            "archetype": ARCHETYPE_BY_NUMBER.get(n, "—"),
            "planet": ARCHETYPE_BY_NUMBER.get(n, "—"),
            "label": label,
            "score": score,
            "advice": advice,
        })
...
def signature_advice(name: str, driver: int) -> Dict[str, Any]:
    ...
    first_arch = ARCHETYPE_BY_NUMBER.get(first_val, "—")
    ...
    SIG_DIRECTION = {
        1: "Upward stroke — confident, clear baseline.",
        2: "Smooth curves — approachable flow.",
        3: "Bold outward strokes — expressive growth.",
        4: "Sharp angles — structured, clean lines.",
        5: "Quick, light strokes — agile signature.",
        6: "Rounded loops — warm, relational feel.",
        7: "Minimal, precise — analytical restraint.",
        8: "Strong descenders — executive weight.",
        9: "Energetic strokes with clear finish.",
    }
    ...
    return {
        ...
        "first_letter_archetype": first_arch,
        "first_letter_planet": first_arch,
...
def implementation_timeline() -> List[Dict[str, str]]:
    ...
    return [
        ...
        {"phase": "Day 8-21",
         "action": "Email signature, business cards, WhatsApp display name — sab jagah update kare. Daily affirmation practice for your Driver number (2–5 min)."},
        {"phase": "Day 22-40",
