...
interface BreakupFactors {
  dasha: Severity;
  houses: Severity;
  venus_moon: Severity;
  kp: Severity;
}
interface BreakupBreakdown {
  dasha: number;
  houses: number;
  venus_moon: number;
  kp: number;
...
interface BreakupResult {
  breakup_score: number;
  risk_level: RiskLevel;
  factors: BreakupFactors;
  reasons: string[];
  breakdown: BreakupBreakdown;
}
...
export default function BreakupChancesScreen() {
  ...
  async function runAnalysis() {
    ...
    try {
      ...
      const resp = await fetch(`${API_BASE}/api/breakup-chances`, {
        method: "POST",
      ...
      setResult(json as BreakupResult);
  ...
  return (
    <CosmicBg>
      ...
      >
        {/* Hero */}
        <View style={s.hero}>
          <LinearGradient colors={["#ef4444", "#f87171"]} start={{ x:0,y:0 }} end={{ x:1,y:1 }} style={s.heroIcon}>
            <Text style={{ fontSize: 28 }}>💔</Text>
          </LinearGradient>
          <Text style={[s.heroTitle, { color: textHi, fontFamily: "Nunito_700Bold" }]}>Breakup Chances</Text>
          <Text style={[s.heroSub, { color: textLo, fontFamily: "Nunito_400Regular" }]}>
            KP + Vedic stress signals across Dasha, Houses & Transits
          </Text>
        </View>
        ...
        {canAnalyze && (
          <View style={[s.pairCard, { backgroundColor: bgCard, borderColor: border }]}>
            <PersonChip label="You" name={primaryProfile!.name} place={primaryProfile!.birthData?.place}
              color="#ef4444" textHi={textHi} textLo={textLo} bgCard2={bgCard2} border={border} />
            <View style={s.pairDivider}>
              <Text style={{ fontSize: 18 }}>💔</Text>
            </View>
            <PersonChip label="Partner" name={partnerProfile!.name} place={partnerProfile!.birthData?.place}
              color="#f87171" textHi={textHi} textLo={textLo} bgCard2={bgCard2} border={border} />
          </View>
        )}
        ...
        {canAnalyze && loading && (
          <View style={[s.loadingCard, { backgroundColor: bgCard, borderColor: border }]}>
            ...
            <Text style={[s.loadingText, { color: textHi }]}>Reading stress signatures…</Text>
            <Text style={[s.loadingSub, { color: textLo }]}>
              Dasha · Houses · Venus-Moon · KP Sub-lord · Transit
            </Text>
          </View>
        )}
...
function riskBlurb(risk: RiskLevel): string {
  if (risk === "low") return "Stable — minimal breakup pressure";
  if (risk === "medium") return "Moderate — manageable friction";
  if (risk === "high") return "High risk — active stress signals";
  return "Very high — strong separation pattern";
}
function sevColor(sev: Severity): string {
  return sev === "severe" ? "#ef4444" : sev === "moderate" ? "#f59e0b" : "#22c55e";
}

function ResultView({
  result, accent, textHi, textLo, bgCard, bgCard2, border, isDark, onReAnalyze,
}: {
  result: BreakupResult; accent: string; textHi: string; textLo: string;
  bgCard: string; bgCard2: string; border: string; isDark: boolean;
  onReAnalyze: () => void;
}) {
  const R = 70;
  const circ = 2 * Math.PI * R;
  const frac = Math.max(0, Math.min(1, result.breakup_score / 100));
  const offset = circ * (1 - frac);
  const scoreCol = riskColor(result.risk_level);

  const FACTOR_ORDER: (keyof BreakupFactors)[] = ["dasha", "houses", "venus_moon", "kp"];
  const FACTOR_LABEL: Record<keyof BreakupFactors, string> = {
    dasha: "Dasha timing",
    houses: "House affliction",
    venus_moon: "Venus / Moon",
    kp: "KP sub-lord",
  };
  const FACTOR_EMOJI: Record<keyof BreakupFactors, string> = {
    dasha: "⏳", houses: "🏛️", venus_moon: "🌙", kp: "🔭",
  };

  // Dasha and sub-sections — show with max contribution caps
  const BREAK_ROWS: [string, number, number][] = [
    ["Dasha timing",  result.breakdown.dasha,      60],
    ["House affliction", result.breakdown.houses,  25],
    ["Venus / Moon",  result.breakdown.venus_moon, 10],
    ["KP sub-lord",   result.breakdown.kp,         5 ],
    ["Transit adj.",  result.breakdown.transit,    10],
  ];
  ...
  return (
    <View style={{ gap: 14 }}>
        ...
        <View style={s.scoreWrap}>
          ...
          <View style={s.scoreTextWrap}>
            <Text style={[s.scoreText, { color: scoreCol, fontFamily: "Nunito_700Bold" }]}>
              {result.breakup_score}
            </Text>
            <Text style={[s.scoreSub, { color: textLo }]}>breakup risk</Text>
          </View>
        </View>

        <View style={[s.riskBadge, { backgroundColor: scoreCol + "22", borderColor: scoreCol + "66" }]}>
          <Feather name="alert-triangle" size={13} color={scoreCol} />
          <Text style={{ color: scoreCol, fontFamily: "Nunito_700Bold", fontSize: 13, letterSpacing: 0.3, textTransform: "uppercase" }}>
            {result.risk_level}
          </Text>
        </View>
        <Text style={[s.verdict, { color: textHi, fontFamily: "Nunito_600SemiBold" }]}>
          {riskBlurb(result.risk_level)}
        </Text>
      </View>

      {/* Factor pills */}
      <View style={[s.resCard, { backgroundColor: bgCard, borderColor: border }]}>
        <Text style={[s.sectionTitle, { color: textHi }]}>Stress factors</Text>
            ...
            return (
              <View key={k} style={[s.factorPill, { backgroundColor: bgCard2, borderColor: col + "55" }]}>
                ...
                <View style={{ flex: 1 }}>
                  ...
                  <Text style={{ color: col, fontSize: 11, fontFamily: "Nunito_600SemiBold", textTransform: "capitalize" }}>
                    {v}
                  </Text>
                </View>
              </View>
      ...
      <View style={[s.resCard, { backgroundColor: bgCard, borderColor: border }]}>
        ...
        {BREAK_ROWS.map(([lbl, val, max]) => {
          const absVal = Math.abs(val);
          const pct = Math.min(100, Math.round((absVal / max) * 100));
          const col = val < 0 ? "#22c55e" : val === 0 ? "#64748B" : val >= max * 0.66 ? "#ef4444" : val >= max * 0.33 ? "#f59e0b" : "#64748B";
          return (
            <View key={lbl} style={s.barRow}>
              ...
              <Text style={[s.barVal, { color: textHi }]}>{val > 0 ? `+${val}` : val}</Text>
            </View>
          );
        })}
        <View style={[s.capsHint, { borderColor: border }]}>
          <Feather name="info" size={11} color={textLo} />
          <Text style={{ color: textLo, fontSize: 11, fontFamily: "Nunito_500Medium", flex: 1 }}>
            Weights — Dasha 60 · Houses 25 · Venus-Moon 10 · KP 5 · Transit ±10
          </Text>
        </View>
      </View>
        ...
        <View style={[s.resCard, { backgroundColor: bgCard, borderColor: border }]}>
          ...
          <View style={{ gap: 8 }}>
            {result.reasons.map((r, i) => {
              ...
              const positive = /supportive|reducing|protection|healing|not tied|no kp separation|grounded|warmth/.test(lower);
              const negative = /activates break|amplifies|afflict|weak|debilit|karmic confusion|dusthana|separation|obsess|detach|strain|coldness|fight|anger|conflict|delay|confus|damage/.test(lower);
...
const s = StyleSheet.create({
  ...
  heroSub: { fontSize: 12.5, letterSpacing: 0.2, textAlign: "center", maxWidth: 300 },
  ...
  verdict: { fontSize: 14, textAlign: "center", letterSpacing: -0.1, marginTop: 6 },

  riskBadge: { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6,
  ...
  barLbl: { width: 115, fontSize: 12, fontFamily: "Nunito_600SemiBold" },
  barTrack: { flex: 1, height: 8, borderRadius: 4, overflow: "hidden" },
  barFill: { height: "100%", borderRadius: 4 },
  barVal: { width: 36, textAlign: "right", fontSize: 12.5, fontFamily: "Nunito_700Bold" },

  capsHint: { flexDirection: "row", alignItems: "center", gap: 6, marginTop: 6, paddingTop: 10, borderTopWidth: 1 },
