...
export function MarriageCompatProPurchase({
  isDark,
  primaryName,
  partnerName,
  priorityDelivery,
  ...
  const copy = milanProPurchaseCopy(lane);
  ...
  return (
    <Animated.View style={{ opacity: fadeAnim, gap: 10 }}>
      ...
      <View style={[s.heroCard, { borderColor: isDark ? "rgba(124,58,237,0.5)" : "rgba(124,58,237,0.35)" }]}>
        <LinearGradient
          colors={isDark ? ["rgba(124,58,237,0.22)", "rgba(99,102,241,0.14)"] : ["rgba(124,58,237,0.1)", "rgba(99,102,241,0.06)"]}
      ...
      <View style={[s.card, { backgroundColor: cardBg, borderColor: border }]}>
        ...
        >
          {FOUNDER_PROFILE.photoUri ? (
            ...
            <LinearGradient colors={["#6366f1", "#7c3aed"]} style={s.founderPhoto}>
        ...
        <View style={s.coreQChipRow}>
          {copy.coreQuestions.map((q, i) => (
            <View key={q} style={[s.coreQChip, { borderColor: border }]}>
              <Text style={[s.coreQChipNum, { color: isDark ? "#a78bfa" : "#7c3aed" }]}>{i + 1}</Text>
...
const s = StyleSheet.create({
  partnerBanner: { paddingVertical: 8, paddingHorizontal: 10, borderRadius: 12, borderWidth: 1 },
  ...
  reportChip: { flexDirection: "row", alignItems: "center", gap: 4, paddingVertical: 5, paddingHorizontal: 8, borderRadius: 8, borderWidth: 1, maxWidth: "48%", flexGrow: 1 },
