...
- **Unified Mega-Prompt (2026-05-06)**: One single system prompt covers all topics — LLM picks the relevant checklist based on the user's question (no separate prompts per topic). Embeds: (a) Core reading method = D1 placement read + **mandatory D9 cross-check** rule (where does D1's significator lord sit in D9? D9-strong = result matures/sustains; D9-weak = result fragile even if D1 looks good); (b) Topic checklist with houses+lords+karakas for wealth, marriage, love-vs-arranged, career, health, children, education, property, foreign, spirituality, litigation, parents, siblings, longevity; (c) STATIC vs TIMING output rule injected per request. Defined inline in `raw_passthrough_ask()`.
