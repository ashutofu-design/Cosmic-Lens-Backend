...
## Latest Phase
- **Phase 2.5.11.23 — Kundli Milan Pro PDF (May 8 2026)**: Premium 24-page "Cosmic Relationship Blueprint Pro". 5 new `vedic/compat/` modules (d9_marriage, synastry_7l, kp_marriage_promise, chapter_scores, premium_chapters with `gpt-4o` polish + always-safe fallback + two-tier cache). `milan_pdf.py::render_milan_pro_pdf()` 24-page A4 skeleton. New endpoint `POST /api/kundli-milan/pro-pdf` accepts BOTH `{p1, p2, lang}` (mobile-friendly) and `{milan, kundli_p1, kundli_p2, lang}` (advanced). Mobile `handleDownloadProPdf` now single-call. 63 tests green; 3-lang live smoke (en=35890B, hi=57758B Devanagari, hn=35921B; all 24pp). Full prior phase history in `docs/engines.md`.
