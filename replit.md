...
- **Kundli Milan LLM Prose Polish — Phase 2.5.11.20 (May 7 2026)**: User feedback "compatibility text robotic feel deta hai, LLM se sundr likhwa do". Implemented HYBRID layer over deterministic Ashtakoot facts: engine still computes all numbers (Swiss Ephemeris) + rule-based templates (preserved as fallback); new `vedic/compat/llm_polish.py` rewrites the 4 narrative sections (`compatibility_insight`, `strengths`, `challenges`, `marriage_outlook`) in a "mature Vedic astrologer" voice. **Architecture**: prompt builder + `gpt-4o-mini` call + 5-rule validator + fact-fingerprint LRU cache (1024 entries). Validator enforces: verbatim total citation, per-partner anchor (nakshatra OR rashi), remedy-keyword in every challenge, banned-term ban (lifespan/death/gender/guarantees), length sanity. **Whitelist** `ALLOWED_REMEDIES` (11 entries) prevents random gemstone/tantrik suggestions. **Fallback path**: any failure (validator reject, LLM down, JSON parse error) silently returns rule-based templates → user never sees an error. **Toggles**: `COMPAT_LLM_POLISH=1` (default off), `COMPAT_LLM_MODEL=gpt-4o-mini`, `COMPAT_LLM_CACHE_SIZE=1024`. Wired into `flask_app.py /api/kundli-milan` exit (line 8815). 17 unit tests (validator + cache + toggle-off + mocked-LLM happy/sad paths) + 3 live LLM smokes (low/high/mid-compat charts) — all pass, prose quality genuinely warm + factual. Cost: ~$0.0004 per call (gpt-4o-mini, ~1.2k tokens), with 90%+ cache hit rate at scale.
