...
- **CAFB-health full migration (2026-05-06)**: `health_focus_routing.py` (905L, mirrors `property_focus_routing.py`) replaced the deleted `health_static/` engine + root `health_engine.py`/`health_rules.py`/`health_topics.json`/`health_recipe_composer.py`/`health_topic_matcher.py`/`health_routing.py` (~400 KB removed). Architecture: ~26 atomic CHECK blocks (~150 chars each) + framework header teaching LLM to detect axes server-side, compose 2-4 atomic blocks, ignore the rest. **Axes**: ACTION × SYSTEM (digestive/cardio/nervous/musculo/skin/endocrine/respiratory/immune/mental/repro) × INTENT (STATIC/QUALITY/TIMING/RISK/REMEDY) × SEVERITY. **Hard guards** (precedence: crisis → recovery → decline): `REFUSE_DIAGNOSIS`, `REFUSE_DEATH`, `REFUSE_CURE_GUARANTEE`, `CRISIS_REDIRECT` (suicide/self-harm), `REFUSE_TIMING_DECLINE`, `REFUSE_TIMING_RECOVERY`, `REFUSE_SURGERY_MUHURAT`. **Sensitive bucket detector**: mental_health/reproductive/parent_health/addiction → softer tone + extra disclaimer. **Mandatory medical-disclaimer post-injector** (`apply_health_postinjectors`) wired at narrative + sync `/api/ask` + stream `/api/ask/stream`. **Killswitches**: `HEALTH_FOCUS_BLOCK` / `HEALTH_FOCUS_AXES` / `HEALTH_CHART_SLICE` / `HEALTH_DISCLAIMER` (all default ON). `HEALTH_STATIC_BYPASS=1` default ON in `flask_app.py`. Tests: 56/56 passing in `test_health_focus_routing.py`. Smoke: 15 health Q types verified end-to-end (digestive/cardio/mental/reproductive/skin/endocrine/respiratory/immune/musculo/eyes/death-refusal/crisis-care/surgery/general). Note: under default `RAW_PASSTHROUGH_MODE=1` the CAFB engine is dormant — the mega-prompt's HEALTH topic checklist handles all 15 directly. CAFB activates when `RAW_PASSTHROUGH_MODE=0` (legacy pipeline).
- **KP opt-in enrichment for raw_passthrough (2026-05-06)**: New `_KP_TRIGGER_RE` regex + `_raw_kp_block(kundli)` helper in `openai_helper.py`. Default `/api/ask` flow stays D1+D9 only (~1012 chart_chars, ~98% prompt cache hit). When the question contains explicit KP terminology (`kp`, `cusp`, `sub-lord`, `csl`, `krishnamurti`, `prashna`, `ruling planet`) **OR** hard yes/no event markers (`hoga ya nahi`, `lagega ya nahi`, `milegi ya nahi`, `pakka batao`, `yes or no`, `definite`, `promise`, `fructify`), append the cuspal-sub-lord verdict block (~975 chars) to chart context — chart_chars grows to ~1988. **Fast path**: uses cached `kundli["kp"]` (cusps + significations) directly, bypassing Swiss-Ephemeris recompute (the legacy `_phase55_safe_compute_kp_summary` requires lat/lon/tz on the kundli root which `/api/kundli` doesn't echo back). **KP reading rule** injected only when block present: KP wins on hard yes/no event timing, D1/D9 wins on quality/character. **Translation**: NEVER mention `KP`/`cusp`/`sub-lord` in user-facing reply — translates per GOLDEN RULE. Telemetry: `[raw_passthrough] kp=True/False`. Verified: 3 trigger Qs fire KP, 3 default Qs unchanged, zero jargon leak.
- **Cosmic Intelligence rebrand (2026-05-06)**: All "Acharya Vidyasagar" persona references removed from mobile Ask tab. Header → "Cosmic Intelligence" + cpu icon + green live-dot + "Vedic chart analyst · Online". Landing → premium pill badge "Cosmic Advance Intelligence", "How can I help today?", evidence-based subhead. DEMO_MESSAGES + STARTERS + init message + mode card body + AcharyaTypingDots caption + MessageActionsSheet share dialog all rebranded. Word "AI" banned everywhere — only "Cosmic Advance Intelligence" / "Cosmic Intelligence" used. Server-side legacy Acharya prompt blocks at L4674/5407/5505/16981 dormant under `RAW_PASSTHROUGH_MODE=1`.
...
- **User-Facing Translation Layer (2026-05-06, GOLDEN RULE)**: Mega-prompt now has highest-priority "translation" block that BANS all astrology jargon from user-facing output — no planet names (Mars/Mercury/etc.), no house numbers (1H/7L), no signs (Cancer/Scorpio), no nakshatras (Mula/Ardra), no D1/D9 mentions, no aspect/drishti/dasha/yoga technical terms (except yoga name when user asked it by name e.g. "Manglik hu kya"). Topic checklists + yoga library + special aspects + cancellation rules become INTERNAL REASONING ONLY. Output uses plain-language vocabulary: "body energy", "stress reaction", "digestion sensitivity", "partnership ease", "career stability", "next 6-12 months". Tone calmed: banned fear words ("danger", "lamba khinchne", "destroy") replaced with soft framing ("thodi sensitivity", "extra dhyan", "balance helpful"). Disease-name predictions banned — say "digestion sensitivity" not "acidity". MANDATORY `👉 Final:` 1-line plain-language verdict at end of every single-topic and multi-topic answer (skipped only for 1-line yes/no Qs). Verified across 5 Q types: zero jargon leak, all have 👉 Final.
- **Length Discipline STRICT (2026-05-06)**: Mega-prompt OUTPUT RULES tier answer length to Q complexity. **Yes/no factual Q** → MAX 1-2 lines, single line, verdict + ONE reason. **Single-topic Q** → MAX 3-4 lines, ~60 words, ONE paragraph (no separate "Risk Areas"/"Recovery"/"D9 me…" paras). **Multi-topic/full-chart Q** → up to ~250 words structured. Length rule explicitly OVERRIDES "D9 mandatory" + "cite cancellations" rules for short tiers. Verified: yes/no Qs ~80-170 chars, single-topic ~80 words, full-chart ~2000+ chars. Avg ~₹0.10/Q vs prior ~₹0.44/Q (~70% cheaper).
- **Health Topic Expansion (2026-05-06)**: HEALTH entry in mega-prompt topic checklist now includes: (a) CORE FRAMEWORK — vitality (1H+1L+Sun+Moon), acute disease (6H+6L+Mars+Saturn+Rahu), chronic/surgical (8H+8L), karma-to-body (12H), current dasha lord, Sade Sati, D9 of 1L; (b) BODY-SYSTEM CHECKLIST (10 systems) — mental/nervous, digestive, cardio, reproductive, musculo-skeletal, skin/blood, endocrine, respiratory, immune, eyes/ears — each with specific houses + planets + nakshatras to scan; LLM picks ONLY the system question implies (or top-2 risk for generic Q); (c) HARD GUARDS — tendencies-not-diagnosis, no death year, mental-distress care, mandatory practical lifestyle line at end. Verified: digestive Q correctly cites Mars+5H+Virgo signal, mental Q correctly cites Moon+12H+Mula, generic future-health Q picks top-2 (digestive+mental).
- **Prompt-cache telemetry (2026-05-06)**: `[raw_passthrough]` log line includes `prompt=X cached=Y (Z%) completion=W`. OpenAI auto-caches static prefix; observed 94-95% cache hit on 2nd+ identical-prefix request. Cached input billed at ~25% of normal rate.
