...
- **Remedy Engine v1.0 (Phase 2.2, May 6 2026)**: Standalone hybrid 3-tier engine at `remedy/` (catalog + remedy_engine_v1 + conflict_check + substitutions + stack_builder). Public API `get_remedies(topic, planets, areas, severity, user_facts)` + `render_for_locked_facts(result)`. Covers health/marriage/career × 9 grahas × 3 tiers (PRACTICAL first → AYURVEDIC → VEDIC last). Anti-superstition: vedic never alone. Every entry carries KPI + cost ballpark + caveats. Health engine delegates via `_compute_health_remedies`. Marriage uses `top_marriage_planets`, career uses `house_lords`. All 3 emit parallel `▸ <TOPIC> REMEDIES` blocks via `locked_facts.py`.
