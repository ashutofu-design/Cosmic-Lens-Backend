...
## Phase H2.2 — TONE-GUARD 2-LAYER (LLM PROMPT + VALIDATOR SCRUB) (2026-05-05)

**Gap caught by user (post-H2.1)**: H2.1 removed the structural doctor disclaimer but left the LLM (NARRATIVE/HYBRID) free to organically write "professional se baat karo / expert guidance lo / therapist consult karein", silently bypassing the doctor-mention policy and creating inconsistent UX. Insight: **finance had hallucination problem; health has tone-drift problem** (different failure mode → different fix).

**Fix — 2-layer defense-in-depth**:

**Layer 1 — Prompt guard (primary)**: Added Rule #12 (TONE-GUARD CRITICAL) to `_llm_narrative` system prompt in `health_static/health_replies.py`. Explicit ban on suggesting doctor / professional / therapist / expert / specialist / counsellor / medical or clinical consultation. Bilingual coverage ("doctor se milein", "professional se baat karo", "expert guidance lo", "mental-health professional", etc.). Reinforces engine's positioning as preventive insight, NOT doctor-replacement, and tells LLM the referral line is auto-added by reply layer on WARNING routes — narrative MUST NOT include it. Pivots tone to self-action (sleep / hydration / breathing / journaling / walking / talking to a trusted friend / lifestyle tweaks). Existing rules renumbered (12 → 13).

**Layer 2 — Validator scrub (backup)**: Added `_REFERRAL_TRIGGER_WORDS` + `_REFERRAL_SENTENCE_RX` + `_REFERRAL_TRIGGER_RX` in `health_static/validator.py`. New step 11.5 in `validate_health_llm_output()` runs ONLY when `add_doctor=False` (i.e. all non-WARNING modes). Strategy: drop the entire **sentence** containing the trigger word (cleaner than mid-sentence noun swap which produces awkward grammar). Belt-and-suspenders: any residual trigger word that survives sentence-drop (mid-clause embeddings) gets neutralised inline → "trusted person". Trigger vocabulary covers: doctor, physician, therapist, counsellor/counselor, (mental-health) professional, specialist, expert (guidance/advice/consult), medical (advice/consultation/help/professional), clinical (help/consult/advice), psychiatrist, psychologist. Two flags: `referral_scrubbed` (sentence dropped) + `referral_neutralised_inline` (residual word swapped). Both promoted to `hard_clean` action tier in `hard_flags` set. WARNING path (`add_doctor=True`) skips this scrub entirely so timing/serious replies retain "doctor consult karein" wording.

**8-case validator unit tests**: doctor / professional / expert guidance / therapist / specialist / counsellor / mental-health professional / clean control — all 8 PASS (referral words removed cleanly, control text untouched, "Final:" line preserved). WARNING preservation test: `add_doctor=True` keeps "Doctor consult karein zaroor." intact ✅.

**E2E P40 NARRATIVE mental_health regression**: prompt-guard alone now prevents LLM from writing referral words (no validator-scrub trigger needed in this run). Output pivots organically to self-action ("raat ko sone se 30 minute pehle screen band karke 10 minute deep breathing aur 5 minute journaling shuru karo") — exactly the supportive/self-action tone the policy demands. Zero referral leaks across {doctor, professional, therapist, expert, specialist, counsellor, psychiatrist, psychologist}.
