...
- **P1.2.10 Gap-Fix Wave** (post-architect refinements): **Gap-2 lexicon widening + conflict guard** — `domain_splitter._DOMAIN_KEYWORDS` re-added safe-but-useful terms (finance += `business`, `dukaan`, `dukan`, `shop`, `venture`, `startup`, `freelance income`; dasha += `gochar`, `vimshottari`, `yogini dasha`). To prevent collision regressions architect flagged (`business job kab milegi` falsely splitting into finance+career), added `_apply_conflict_guards()` post-filter in `extract_domains`: if a domain's matched keywords are ALL in `_FINANCE_WEAK_TOKENS` set AND a competing strong-marker domain (career) is present, the weak hit is dropped. Result: `business job`, `startup job switch`, `dukaan me promotion` → career-only (no false multi); but `Mera business kaisa chalega` (no career marker) → finance-only ✅; and `Job aur paisa dono` (strong job + strong paisa) → genuine career+finance multi ✅. **Gap-3 anon-quota self-heal** — `anon_rate_limit._init(force=True)` parameter + `sqlite3.OperationalError` recovery branch in `check_anon_quota`: if process-cached `_INITED` flag lies (e.g., DB file deleted out-from-under via `rm`), missing-table errors trigger one force-reinit + retry. Logs `[anon_rate_limit] recovered from missing-table via force-reinit`. **Gap-5 ack >3 secondaries** — `intent_splitter._MAX_SECONDARIES` bumped 3→5; `build_acknowledge_line()` now appends lang-aware `(+N aur)` / `(+N more)` suffix when 4-5 secondaries exist (first 3 named explicitly, rest summarised). User sees confirmation that ALL their topics were noticed. **Gap-6 length cap relaxed** — `question_length_gate.MAX_QUESTION_CHARS` bumped 300→500 (~80 words); `SOFT_WARN_CHARS` 200→350. Genuine multi-paragraph context Qs no longer hard-rejected. Killswitches unchanged. All 4 fixes verified via unit + HTTP smoke. Known deferred (NOT fixed in this wave): topic-router false routing (`commercial property invest` → Finance route), gpt-5 temperature dormancy, mixed-script edge cases.
- **Multi-Intent Splitter B1** (`domain_splitter.py` + `intent_splitter.py`, P1.2.10): Two-part fix for cross-domain Q failures (H1 silent-drop, H2 false off-topic, H3 silent-drop). **Part A — `domain_splitter.py` (~265 LOC)**: deterministic regex extractor over 12 jyotish domains (property/marriage/career/finance/health/education/family/vastu/dasha/remedies/travel/legal). Strict word-boundary patterns; case-insensitive; returns `[DomainHit]` sorted by confidence desc. Killswitch: `DOMAIN_SPLITTER=off`. **Part B — `intent_splitter.py` (~170 LOC)**: B1 orchestrator — when ≥2 distinct domains hit, returns `MultiIntent(primary, secondaries)`. `build_acknowledge_line()` produces lang-aware (hinglish/hi/en) 1-line ack template; `is_already_acknowledged()` heuristic for idempotency. Killswitch: `MULTI_INTENT_SPLIT=off`. **Wiring**: `_maybe_inject_multi_intent_ack()` helper (openai_helper.py L6910) called as post-injector at 3 chokepoints in flask_app.py — main route return (L6255), 8× engine early-returns via replace_all (`_log_question_history` pattern, L6017+6056+6108+6140+6513+6549+6589+6617), SSE `kind: final` event (L6722). Helper is idempotent (skips if `is_already_acknowledged` matches). **Brand-guard escape hatch** (`_is_brand_unsafe` L6869): when negative-list pattern matches but `domain_splitter.is_jyotish_anchored(question)` returns True, bypass the off-topic refusal. Fixes H2 false-reject ("share market...aaj kal..." matched stock-prediction regex but is genuinely jyotish-anchored to property+health). Killswitch: `BRAND_GUARD_POSITIVE_ESCAPE=off`. **Strategy = B1 ("Primary + Quick Mention")**: LLM answers focused on PRIMARY domain (~1× tokens, P1.2.9 savings preserved); engine appends 1-line note naming secondaries so user knows they were noticed. Token-flat, no frontend changes, single quota slot. 65/65 unit tests + H1/H2/H3 smoke 3/3 ack-injected ✅ + brand_guard escape verified in logs (`P1210_ESCAPE: domains=['finance','property','health']`).
