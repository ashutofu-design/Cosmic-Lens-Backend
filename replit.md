...
- **Length Discipline (2026-05-06)**: Mega-prompt OUTPUT RULES instruct LLM to scale answer length to question complexity — yes/no Qs → 1-2 lines, single-topic → 3-6 lines, multi-topic/full-chart → up to ~250 words. Replaces fixed "180-250 words" rule. `RAW_PASSTHROUGH_MAX_TOKENS=700` retained as safety ceiling. Result: avg output dropped from ~530 to ~250-400 tokens depending on Q mix → ~50% cost reduction without quality loss.
