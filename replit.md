...
- **Property Focus Block (P1.1)**: Per-topic analysis checklist injected into LLM system prompt for property/ghar Qs. Single source of truth `_PROPERTY_FOCUS_TEXT` + helpers `_property_focus_enabled()` / `_is_property_topic()` / `_passthrough_property_focus()` defined near `_passthrough_marriage_block` (~L238). **Wired at all 4 LLM call sites for parity** (since `LLM_FULL_CHART_MODE` defaults ON → passthrough is dominant path): narrative `_build_messages` (~L4424 elif), sync passthrough (~L14336), stream passthrough (~L17924), legacy passthrough (~L3784). Topic detection: `topic_id ∈ {property, home_property, real_estate, ghar}` OR regex fallback on question keywords (`ghar|makaan|property|plot|flat|jameen|home|house|real estate`). Tells GPT what to check (D1: 4H/4L/karakas/yogas/risk; D9: navamsa dignity, vargottama; KP: 4th/11th/12th CSL significations) and how to answer (3-4 most relevant points, cite actual chart values, 100-150 words Hinglish, refuse timing Qs). Doctrinal hedging on vargottama (supportive ≠ guarantee), neecha-bhanga (requires cancellation conditions), KP 4/11/12 (core is 4 + 11 OR 2; don't dogmatically demand all). Killswitch `PROPERTY_FOCUS_BLOCK ∈ {0,false,no,off}`, default ON. Pairs with P1.0-disable + KP-ALWAYS-FULL.
