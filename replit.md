...
- **Signal-Pack Question-Type Registry (H2.7.18)**: Each registered question_type has its own (a) regex detector, (b) single-source classifier function (truth-table on relevant dims), (c) signal-pack builder branch. **Currently registered**: `compare_immunity_vs_lifestyle` (truth-table on vit×dr) + `vague_discomfort_why` (truth-table on vit×mh, for "ajeeb sa feel kyun" / "bechaini kyun" type questions — strict input discipline rule prevents LLM from inventing symptom-lists). Routing layer (`health_routing.py`) updated to recognize `bechain[ai]?`, `ajeeb`, `uneasy`, `khali sa`, `theek nahi lagta` as health-topic keywords. Cache namespace bumped v10 → v11.
