...
- **Marriage Engine wiring (VIVAH-7) in raw_passthrough**: `_passthrough_marriage_block` (calls `event_timing.marriage.assess_marriage`) is invoked from `raw_passthrough_ask` ONLY when `_M17_MARRIAGE_KW_RX` matches the question (cost-gate). Returns top-3 KP-gated × Jupiter-trigger × Ashtakavarga DBA windows + verdict + risk_flags as a LOCKED-FACTS block the LLM must quote verbatim (dates exact, dasha labels translated). When marriage block fires, `kp_reading_rule` is suppressed (single-authority — marriage engine internally KP-gates). Chart_label gets `+ MARRIAGE ENGINE (VIVAH-7)`. Telemetry `[raw_passthrough] marriage=True/False`.
- **Marriage windows min-gap filter (Phase 2.10.1)**: Both `_select_diverse_top3` (main score-path) and `_chronological_dtt_top3_strict` (chrono path) in `marriage_timing.py` enforce `_MIN_WINDOW_GAP_DAYS = 60` (~2 months) edge-to-edge between every selected top-3 window. Reason: prevents same-DBA-phase double-counting + back-to-back/overlapping windows that look cosmetic but reduce accuracy. Fallback to ungated pool only if all candidates fail the gap (so top_3 never goes empty when valid candidates exist).
