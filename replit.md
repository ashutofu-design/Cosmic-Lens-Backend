...
- **Architecture**:
    ...
    - **Risk Text Engine** (`risk_text_engine.py` + `/api/risk-radar` enrichment): Replaces hardcoded mobile RISK_BY_LEVEL templates and BEST_TIME_SLOTS arrays with real engine output. `detect_dominant_trigger()` mirrors `compute_risk_radar`'s severity ordering (Volatile=10, Chandrashtama=9, Tara Naidhana=8, Saturn Heavy=8, Mars Active=7, Tara Mild=6, PD Weak=6, Tithi Rikta=6 [precedence over Amavasya], Amavasya=6, Saturn Mild=5, Rahukal=4) over 12 trigger keys. Each trigger maps to a 5-field Hinglish text bundle (kya_risk_hai / kya_dhyan_rakhna_hai / kya_avoid_karna_hai / kya_karna_hai / upay) + category label. `build_choghadiya_schedule()` produces 16 day+night segments with quality (best/neutral/avoid) reusing energy_engine's `_DAY_CHOGHADIYA`/`_NIGHT_CHOGHADIYA`/`_RAHUKAL_SEGMENT`. `pick_best_avoid_times()` selects the next-upcoming Amrit/Shubh/Labh as BEST and Rahukaal-or-next-Rog/Kaal/Udveg as AVOID. Sunrise/sunset uses `_astronomical_sun_times` (NOAA SPA when birth lat/lon present) or `_seasonal_sun_times` fallback. The Flask endpoint always emits `enriched: true|false` flag so the client can detect partial failures. Mobile `lib/riskTextAPI.ts` + `RiskRadarCard.tsx` apply a STRICT no-template-fallback gate: Day-1 only overrides when `enriched && top_risk && best_time && avoid_time` are all present; otherwise shows explicit "Cosmic signals abhi fetch nahi ho paaye" unavailable state with "—" placeholders — never the hardcoded RISK_BY_LEVEL template. Days 2-7 keep the deterministic template (intentional, not claimed as real-time). The "✨ Powered by Advanced Cosmic Intelligence" footer renders only when the strict apiOk gate passes.
