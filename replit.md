...
- **Length Discipline STRICT (2026-05-06)**: Mega-prompt OUTPUT RULES tier answer length to Q complexity. **Yes/no factual Q** → MAX 1-2 lines (verdict + ONE strongest reason; D9/cancellation/2nd-confirmation explicitly suppressed via "OVERRIDES all other rules" clause). **Single-topic** → 3-6 lines. **Multi-topic/full-chart** → up to ~250 words structured. `RAW_PASSTHROUGH_MAX_TOKENS=700` retained as safety ceiling. Verified: yes/no Qs now 50-170 chars (was ~600), avg ~₹0.10/Q vs prior ~₹0.44/Q (~70% cheaper).
